...
Creates a new email template record in Dynamics 365. Called from the `create_email_template` widget when the user selects Save. Not intended for direct invocation by the agent—the widget manages the save flow.
...
| Input | Description | Required |
|---|---|---|
| `templateTypeCode` | `templateTypeCode` (Dataverse templatetypecode picklist value, for example, 112 for Case), `languageCode` (LCID, for example, 1033 for English—defaults to 1033). | No |
...
- Internal widget use only—called when the user selects Save in the template creation form.
...
This tool creates a new email template record in Dataverse. Microsoft 365 Copilot prompts the user for confirmation before it runs.
