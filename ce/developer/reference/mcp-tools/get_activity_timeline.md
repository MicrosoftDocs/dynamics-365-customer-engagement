...
> [!TIP]
> You can take action directly from the timeline. Select an email to read it, or use prompts like "reply to the last email" or "create a follow-up task."
...
**No, viewing the timeline doesn't change data.**
...
| Input | Description | Required |
|---|---|---|
| `typeFilter` | `typeFilter` (array of activity types to include, for example, `['email', 'task', 'note']`). Omit for all types. | No |
...
This tool only reads and displays timeline data. No data is modified. Follow-up actions from the timeline widget (creating activities, opening emails) may trigger write operations through separate tools.
