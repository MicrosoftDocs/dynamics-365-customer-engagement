...
The assistant removes one of your personal layout preferences by its scope key. After removal, the affected grid, form, or timeline reverts to the profile-level configuration (if one exists), the organization-level configuration, or the system default. This action requires confirmation.
...
- Only your personal preferences are affected. organization-level and profile-level configurations remain unchanged.

## What happens next

- Show my preferences to verify the removal.
- Save a new preference to create a replacement customization.
- What are the organization-level configurations? to see what you'll fall back to.
...
|---|---|---|
| `scope` | `scope` (string, required). The scope key to remove (for example, `grid:incident:*`). Use `list_user_prefs` to discover existing scope keys. | Yes |
...
Permanently removes a personal layout preference for the current user. doesn't affect other users or organization/profile configurations. The assistant asks for confirmation before executing.
