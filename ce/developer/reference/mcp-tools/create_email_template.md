...
Once you're happy with the template, select **Save** in the form to create it in Dynamics 365. The template is then available for future use when composing emails or applying templates to cases.
...
The creation form itself is read-only until you select **Save**. Once saved, the template is stored in Dynamics 365 and available for future use.
...
Opens the email template creation form widget so the user can author a new Dynamics 365 email template. Accepts optional pre-fill values for title, subject, body, and template type. The actual Dataverse write happens through `save_email_template` when the user selects Save in the widget.

## Annotations

| Annotation | Value | Meaning |
|------------|-------|---------|
| `readOnlyHint` | `true` | Microsoft 365 Copilot skips the confirmation prompt. The widget manages persistence through `save_email_template`. |
...
| Input | Description | Required |
|---|---|---|
| `templateTypeCode` | `templateTypeCode` (Dataverse templatetypecode picklist value, for example, 112 for Case, 1 for Account, 2 for Contact, 8 for User). | No |
...
The widget displays a form with fields for template name, subject, body, description, and type. The user fills in or edits the pre-populated fields and selects Save.
