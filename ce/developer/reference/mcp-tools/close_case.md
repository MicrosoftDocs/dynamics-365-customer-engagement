...
- **Updating a field** (for example, status reason without resolution)—route to `update_case`.
...
Closes the case through the Dataverse CloseIncident action, creating a resolution activity record. The case state transitions to Resolved.
