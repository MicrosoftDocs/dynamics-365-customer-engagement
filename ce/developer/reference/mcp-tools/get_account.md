...
The assistant opens a detailed account form for the account you name or specify by ID. You can see all account fields, edit supported fields inline, and navigate to the full form in Dynamics 365.

If you provide an account name and multiple accounts match, the assistant lets you know and suggests using "list accounts" to find the right one.
...
**No, viewing an account doesn't change data.**
...
|------------|-------|---------|
| `readOnlyHint` | Not set | Read-only in practice; this tool doesn't modify data directly. |
...
The form view itself doesn't change data. Inline edits trigger write operations through `update_entity_record`.
