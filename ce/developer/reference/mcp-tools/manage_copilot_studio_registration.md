...
- Always use the Test Connection button before saving to verify the agent is reachable through the Agents SDK.
...
Opening the registration widget doesn't change data. Adding, editing, or removing registrations from within the widget does change your environment's configuration.
...
| Input | Description | Required |
|---|---|---|
| `orgUrl` | `orgUrl` (optional). Overrides the currently selected environment URL when the host doesn't provide one. | No |
