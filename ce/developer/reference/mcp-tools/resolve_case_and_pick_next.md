...
|---|---|---|
| `incidentId`, `caseNumber` | `incidentId` (GUID) or `caseNumber` (for example, CAS-01010-A0A0A0). At least one is required. Identifies the case to resolve. When `caseNumber` is provided, the server resolves it to a GUID. | Yes |
...
Resolves the specified case through CloseIncident and claims the next queue item by assigning worker ID and case ownership.
