...
The assistant saves a layout configuration scoped to a specific Application Profile. This lets you create different layouts for different agent tiers. For example, L1 support agents might see a simplified case grid, while L2 agents see additional technical columns.

Profile configurations override organization-level configurations for agents assigned to that profile.
...
- Profile configurations take precedence over organization-level configurations.
...
- Set up the organization-wide layout to create a baseline all profiles inherit.
...
Creates or updates an agent configuration scoped to an Application Profile. Use for tier-specific customizations where different agent groups need different layouts (for example, different grid columns for L1 vs L2 agents, different form sections visible per profile).
...
| [`save_agent_config`](save_agent_config.md) | Saves configurations at the organization level (all users) |
