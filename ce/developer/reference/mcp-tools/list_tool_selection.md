...
- Use `level: organization` to review organization-wide visibility.
...
|---|---|---|
| `level`, `organization`, `profile` | `level` (`organization` or `profile`, required). Selects whether to inspect organization-wide or profile-level visibility. | Yes |
