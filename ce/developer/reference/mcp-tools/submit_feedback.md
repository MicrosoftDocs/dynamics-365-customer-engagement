...
When the assistant suggests next-best-action steps for a case, you can give feedback if a suggestion is unhelpful or inaccurate. Selecting the thumbs-down button on a suggestion opens a feedback dialog where you can select categories and add comments explaining what was wrong.
...
- You select the thumbs-down icon on a suggested action.
...
Submitting feedback creates a feedback record in the system. This doesn't change case data, but it does record your feedback for quality improvement purposes.
...
| `destructiveHint` | `false` | Feedback can be submitted again; doesn't delete data. |
...
| Input | Description | Required |
|---|---|---|
| `feedbackTypes` | `feedbackTypes` (required). Array of selected feedback categories (for example, `issue`, `suggestion`). | Yes |
