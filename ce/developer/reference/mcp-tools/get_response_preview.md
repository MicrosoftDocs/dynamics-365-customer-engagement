...
| Purpose | Generates a non-email reply (chat message, phone-call talking points, internal note) from a V2 next-best-action step's draft context through the `msdyn_GetAutonomousDraftContent` API |

## Tool behavior

Generates a non-email reply (chat message, phone-call talking points, internal note) from a V2 next-best-action step's draft context through the `msdyn_GetAutonomousDraftContent` API. Returns the composed reply text for inline preview and clipboard copy without creating any Dataverse Activity record.
...
| Input | Description | Required |
|---|---|---|
| `replyActivityType` | `replyActivityType` (required). The non-email reply mode (for example, `chat`, `phonecall`, `task`). Echoed to the widget for display. | Yes |
