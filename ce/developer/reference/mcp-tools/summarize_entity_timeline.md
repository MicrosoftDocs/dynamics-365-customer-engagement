...
- Provide the entity type when asking about non-case records (for example, "summarize the timeline for this account" rather than just "summarize the timeline").
...
The timeline summary is generated from existing activity data and doesn't modify any records.
...
|---|---|---|
| `entityLogicalName` | `entityLogicalName` (required). Dataverse entity logical name of the record (for example, `incident`, `opportunity`, `account`, `contact`, `lead`, or any custom table with Timeline Wall configured). | Yes |
