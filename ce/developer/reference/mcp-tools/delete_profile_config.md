...
Use this capability to remove a layout configuration from a specific Application Profile and revert to the organization-level default.

## What it does

The assistant removes a profile-scoped layout configuration by its scope key. After removal, agents assigned to that profile see the organization-level configuration (if one exists) or the system default. This action requires explicit confirmation.
...
- Revert this profile to the organization default.
...
- organization-level and personal preference configurations are not affected.

## What happens next

- Show profile configurations to verify the removal.
- Set up a new layout for this profile to create a replacement.
- Show organization-level configurations to see which configurations agents use as fallback.
...
Removes an agent configuration from an Application Profile by scope key. Use when the user asks to remove or reset a profile-level customization (for example, "reset L1 grid layout"). Requires explicit confirmation to prevent accidental deletions.
...
| Input | Description | Required |
|---|---|---|
| `scope` | `scope` (string, required). The scope key to remove (for example, `grid:incident:*`). Use `list_profile_configs` to discover existing scope keys. | Yes |
