...
Personal preferences take the highest priority in the configuration stack, overriding both organization-level and profile-level configurations.
...
- Your personal preferences override organization-level and profile-level configurations.
...
Only your own preference record is affected. It doesn't change shared organization-level or other users' settings.
...
Saves a personal layout preference for the current user. Persists across conversations and devices, allowing each user to customize their own agent experience without affecting other users or organization-level configurations. Use when the user says "remember this" or "save as my preference."
...
|---|---|---|
| `entityLogicalName` | `entityLogicalName` (string, required). The entity to configure (for example, `incident`, `account`). | Yes |
...
- The user doesn't have organization admin permissions but wants personal layout tweaks.
...
| [`save_agent_config`](save_agent_config.md) | Saves organization-level configurations (admin only) |
...
Creates or updates a personal layout preference for the current user only. doesn't affect other users.
