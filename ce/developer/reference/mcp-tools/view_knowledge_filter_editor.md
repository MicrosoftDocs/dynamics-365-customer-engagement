...
**Opening the filter editor doesn't change data. Applying or clearing selections in the widget saves user filter preferences.**
...
Renders the knowledge filter editor widget showing current dimensions and selections. The user applies or clears filters through the widget UI, which calls `update_knowledge_filters` internally.
