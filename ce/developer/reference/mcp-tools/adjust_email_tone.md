...
- If you only want to change the length, ask for "shorter" or "longer" without naming a tone, and the assistant keeps the existing tone.
...
- Drive the in-place rewrite path for an open compose draft when invoked through `edit_email_draft` with a `pendingAction`.
