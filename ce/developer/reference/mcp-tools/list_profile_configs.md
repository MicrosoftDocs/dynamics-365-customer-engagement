...
- Use `list_agent_configs` to see organization-level configurations that apply to all profiles.
...
- Show organization-level configurations to compare with organization-wide settings.
...
Lists agent configurations for a specific Application Profile (APM). Profile configurations apply to all agents assigned to that profile, enabling tier-specific layouts (for example, different columns for L1 vs L2 agents). Use when the user asks about layout customizations scoped to a particular profile.
...
- The user asks about organization-wide configurations. Use `list_agent_configs` instead.
...
| [`list_agent_configs`](list_agent_configs.md) | Lists organization-level configurations (applies to all users) |
