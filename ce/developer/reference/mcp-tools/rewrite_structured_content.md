...
Browsing templates and previewing the rewrite doesn't change any records. When you save the final note, it creates an annotation record on the entity's timeline in Dynamics 365.
...
| Input | Description | Required |
|---|---|---|
| `entityLogicalName` | `entityLogicalName` (required). Dataverse logical name of the owning entity (for example, `incident`, `contact`, `account`). | Yes |
