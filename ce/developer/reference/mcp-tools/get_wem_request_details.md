...
The assistant opens a detailed card for a single WEM request. It supports the three WEM request types, with type-appropriate labels and the fields available for each:

- **Time-off requests**—time-off type, whether it's a full day, the start and end of the time off, hours, and notes.
- **Shift-bid requests**—the request's status, dates, and review information.
- **Shift-swap requests**—the request's status, dates, and review information.
...
The assistant displays an interactive request card as an app-in-chat component. The card shows the request's details and header information such as the date submitted, last updated, request status, and owner.
...
- Review the request's details, matched to its type—the time-off window for a time-off request, or status, dates, and review information for a bid or swap.
...
Opens the full detail view of a single Dataverse `msdyn_wemrequest` record. You can identify the record by `recordId` (its GUID) or by `requestName` (its exact name); when `recordId` is omitted, the tool looks the record up by name. If the name matches more than one request, the tool shows the matching requests as a list of cards—so you can select the one to open (it never guesses); if the name matches none, it reports that no request was found. Once a single record is identified, the tool reads the request type and renders a read-only detail card with type-appropriate labels and the core request facts—the time-off window for a time-off request, and the available request facts (status, dates, review information) for a bid or swap. WEM requests have no activities, so no timeline is shown.
...
| `requestName` | The exact request name (`msdyn_name`, for example, "Weekend cover swap"). Used to resolve the record when `recordId` is omitted. When the name matches more than one request, the matching requests are shown as a list of cards so you can select the one to open; agent-based lookup is not supported here (use `list_wem_requests` to find an agent's requests). | No* |

\* At least one of `recordId` or `requestName` is required.

## Response and UI behavior

This tool renders an interactive app-in-chat request card.
...
The card displays the request's details and header fields, matched to the request type.
