---
redirect_url: https://docs.microsoft.com/en-us/powerapps/developer/common-data-service/xrm-tooling/use-xrm-tooling-create-data
...
> [!NOTE]
>  You can also create an entity record using XRM tooling by executing the <xref:Microsoft.Xrm.Sdk.Messages.CreateRequest> message with the <xref:Microsoft.Xrm.Tooling.Connector.CrmServiceClient.ExecuteCrmOrganizationRequest*> method. [!INCLUDE[proc_more_information](../../includes/proc-more-information.md)] [Use messages with the ExecuteCrmOrganizationRequest method](use-messages-executecrmorganizationrequest-method.md)  
  
## CreateAnnotation
  
 This method is used to create and attach a note object to any entity record. While you can populate all the variables for the note in the first pass, you only need to provide subject and note text fields. In practice, this is generally used to attach system-generated notes to an entity, or to attach files that are stored in [!INCLUDE[pn_crm_shortest](../../includes/pn-crm-shortest.md)] to an entity. Additionally, if you provide your own UI for creating notes for your user, this is how you would attach that note to the owner entity in [!INCLUDE[pn_crm_shortest](../../includes/pn-crm-shortest.md)]. This example continues from the prior example to create a note on the newly created account.  
