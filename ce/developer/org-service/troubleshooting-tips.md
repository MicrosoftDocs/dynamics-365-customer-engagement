...
applies_to: 
  - "Dynamics 365 (online)"
helpviewer_keywords: 
  - "CrmSvcUtil.exe, exception when running to generate early-bound entity types"
  - "MessageSecurityException when connecting to the web services, troubleshooting tips"
  - "early-bound entity types, exception when running CrmSvcUtil.exe"
  - "troubleshooting tips, exception when running CrmSvcUtil.exe to generate early-bound entity types"
  - "troubleshooting tips, MessageSecurityException when connecting to the web services"
...
The following information is provided as an aid in diagnosing certain common issues that may arise when developing [!INCLUDE[pn_sdk](../../includes/pn-sdk.md)]–based applications. More content will be added to this topic as time goes on.  
...
```  
 ...
 This error can be caused if the build version of the CrmSvcUtil tool and Microsoft.Xrm.Client.CodeGeneration.dll assembly aren’t the same. When you develop applications using the [!INCLUDE[pn_sdk](../../includes/pn-sdk.md)], always use the CrmSvcUtil tool and assemblies that are provided in the Bin folder of the SDK. [!INCLUDE[sdk_download](../../includes/sdk-download.md)] Don’t use the tool or assemblies from the Tools folder of a [!INCLUDE[pn_dynamics_crm](../../includes/pn-dynamics-crm.md)] server installation.  
