...
## Query Results  
 When you execute a FetchXML query by using the [QueryBase)](https://docs.microsoft.com/dotnet/api/microsoft.xrm.sdk.client.organizationserviceproxy.retrievemultiple\(microsoft.xrm.sdk.query.querybase\)) method, the return value is an <xref:Microsoft.Xrm.Sdk.EntityCollection> that contains the results of the query. You can then iterate through the entity collection. The previous example uses the `foreach` loop to iterate through the result collection of the FetchXML query.  
