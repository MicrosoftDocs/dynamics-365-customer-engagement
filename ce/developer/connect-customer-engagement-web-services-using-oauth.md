...
> [!NOTE]
> All Dynamics 365 for Customer Engagement tools, utilities, and assemblies utilize the authentication patterns supported by Azure Active Directory Authentication Library (ADAL). When writing custom code, you must use ADAL v3.19 (or greater) for authentication if your code uses the XRM Tooling APIs found in the Microsoft.Xrm.Tooling.* namespaces.
> 
> For an example of using ADAL 3 to authenticate, see the GitHub [sample](https://github.com/Microsoft/PowerApps-Samples/tree/master/cds/webapi/C%23/ADALV3WhoAmI). For more information about the benefits of using XRM Tooling APIs in your code see [Build Windows client applications using the XRM tools](/powerapps/developer/common-data-service/xrm-tooling/build-windows-client-applications-xrm-tools).
