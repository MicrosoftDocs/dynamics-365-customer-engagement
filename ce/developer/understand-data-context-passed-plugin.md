...
The <xref:Microsoft.Xrm.Sdk.IPluginExecutionContext.ParentContext> contains information about the operations that may have preceded the current operation. For example another message may create a record. If you register a plug-in on the `Create` message you will find the context of the previous operation in the `ParentContext`.
