...
description: "You should interact with HTTP and HTTPS resources asynchronously."
ms.date: 11/30/2017
...
> [!IMPORTANT]
> Usage of jQuery isn't the preferred approach because it adds a dependency to an external library and isn't recommended in interacting with the product. Refer to [Use of jQuery](https://msdn.microsoft.com/library/hh771584.aspx?#BKMK_UsingjQuery) for more information.

  ```javascript
  // jQuery example that is missing the async property. It defaults to true, which is the value you want.
  var requestXhr = new XMLHttpRequest();
...
The specification for `XMLHttpRequest` states that synchronous usage is being removed from the [standard](https://xhr.spec.whatwg.org/#the-open()-method) because it's now deprecated. The specification encourages that browsers warn when synchronous executions are performed. Currently, browsers are only presenting warnings, but an InvalidAccessError exception might be thrown in the future when a value of false is passed to the async parameter. Modern browsers have deprecated synchronous requests executed on the main thread.
...
Passing the XMLHttpRequest script to the `window.setTimeout` function will break the normal synchronous flow of the page execution, but it will result in the same problem as the synchronous request and will not accomplish parallel processing.
