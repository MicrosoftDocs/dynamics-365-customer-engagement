...
4. Choose **Unified Interface Settings** under the **Advanced Settings**.
...
## Add the Unified Interface Setting record to a Configuration

you can add the Unified Interface Setting record to a configuration in two ways:
...
4. In the **Unified Interface Settings** field, type the name of the existing Unified Interface record you want to assign, and choose the search icon.<br> The record appears. select the record.<br>
![Add unified interface setting record to the configuration](../media/usd-crm-add-unified-interface-record-configuration.PNG "Add unified interface setting record to the configuration")<br>
    >[!Note]
    > In the above step, we added an existing Unified Interface Setting record to the configuration. To create a new Unified Interface Settings record, see [How to create Unified Interface Setting record](#how-to-create-unified-interface-setting-record).

5. Select **Save & Close**.

## Login experience to Unified Service Desk 

The scenarios that you need to consider for Unified Service Desk login are as follows:

### Scenario 1: Users (agents) assigned to Configuration with a Unified Interface Settings record

Add users (agents) to a **Configuration**, and add a record to **Unified Interface Settings** field in the **Configuration**. When the user (agent) added to the configuration signs in to Unified Service Desk, the system authenticates the user (agent) and displays the landing page of the Unified Interface App. 
...
Add users (agents) to a **Configuration**, and no Unified Interface Settings records are created. Hence, there are no records to select for the **Unified Interface Settings** field in the **Configuration**. In this scenario, when the user (agent) assigned to a particular configuration signs in to Unified Service desk, the application selection window is display, where the user (agent) has to select the Unified Interface App.
...
## See also
 ...
 [Walkthrough 6: Configure the Debugger hosted control in your agent application](../../unified-service-desk/walkthrough6-unified-interface-configure-debugger-hosted-control-agent-application.md)<br>
