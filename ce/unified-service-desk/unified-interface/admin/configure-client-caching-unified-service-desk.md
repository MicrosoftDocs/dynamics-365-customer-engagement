...
> [!IMPORTANT]
>  This feature has privacy impact because enabling client caching in [!INCLUDE[pn_unified_service_desk](../../../includes/pn-unified-service-desk.md)] leads to some of your data being stored locally on the user’s computer, which is outside the Common Data Service platform services boundary.  
...
4. In the sitemap, choose **Options** under the **Advanced Settings**.
  
5. Select **New** on the **Active UII Options** page.  
  
6. On the **New Option** page, type **ClientCacheVersionNumber** in the **Name** box, and an alphanumeric number in the **Value** box. The alphanumeric value is used as the cache key for [!INCLUDE[pn_unified_service_desk](../../../includes/pn-unified-service-desk.md)].  
...
3. In the site map, choose **User Settings** under the **Advanced Settings**.  
  
5. Select **New** on the **Active User Settings** page.  
  
6. On the **New User Setting** page, specify the following:
 
     | Field | Value |
     |---------------------------|-------------------------------------------------------------------------|
     | User | Type the name, or search and select the name of the user for which you want to disable client caching. |
     | Name | Type **DisableCaching** |
...
7. Select **Save**.
   ...
   Next time, when the user signs in using the [!INCLUDE[pn_unified_service_desk](../../../includes/pn-unified-service-desk.md)] client, client caching isn’t used. However, it doesn’t delete or refresh the client cache store for the user. When the **DisableCaching** key is removed for the user, the user will return to using the previously stored client cache store.  
...
4. In the site map, choose **User Settings** under the **Advanced Settings**.  
...
5. Select the **Related** tab, and then select **UII Actions**.  
...
6. Select **+ Add New UII Action**.  
  
7. Type **ResetLocalCache** in the **Name** field, and then select **Save** on the **New UII Action** page. Close the UII Action and the hosted control.  
  
    Next, we will add an action call to call the UII action that we just created.  
  
8. Select **Action Calls** under **Basic Settings** in the site map.  
  
9. Select **+ New** on the **Active Actions** page.  
  
10. Specify the following on the **New Action Call** page.
