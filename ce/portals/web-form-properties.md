...
| Value             | Value assigned to the attribute when the form is being saved.<br>For Two Option (Boolean) fields, use true or false.<br>For Option Set field, use the integer value for the option.<br>For Lookup (EntityReference) fields, use the GUID.<br>Note that if the attribute is also on the form, the user's value will be overwritten with this value. |                                                                                 |
