...
1. In [!INCLUDE[pn-dynamics-crm](../includes/pn-dynamics-crm.md)], create the custom entity through **Settings** > **Customizations** > **Customize the System** > **Entities**.  In this example, we’ll call the entity “Suggestion”. Set the entity to display in the **Portals** navigation area, then save it.
...
Entity form metadata records contain additional behavior modification logic to augment or override the functionality of form fields that would otherwise not be possible with the native entity form editing capabilities in [!INCLUDE[pn-dynamics-crm](../includes/pn-dynamics-crm.md)].
...
Each completed survey that is submitted will become a record within the **Portals** > **Survey** entity in [!INCLUDE[pn-dynamics-crm](../includes/pn-dynamics-crm.md)], and this record will contain the regarding contact who completed the web form along with all their responses for tabulation.

## Form elements rendering

While most [!INCLUDE[pn-dynamics-crm](../includes/pn-dynamics-crm.md)] forms can be successfully rendered in a portal, there are some call-outs that should be noted as some features aren't supported by the [!INCLUDE[pn-dynamics-crm](../includes/pn-dynamics-crm.md)] EntityFormView control that handles the actual rendering of the entity form.

|Supported form elements|Supported, but with caveats form elements|Unsupported form elements|
|---|---|---|
|All [!INCLUDE[pn-dynamics-crm](../includes/pn-dynamics-crm.md)] field validation rules   |All [!INCLUDE[pn-dynamics-crm](../includes/pn-dynamics-crm.md)] field types, except Party List fields   |Form headers and footers won't display in the portal   |
