---
title: "Walkthrough: Get Dynamics 365 for Customer Engagement data on the portal | MicrosoftDocs"
...
search.app: 
  ...
  - D365Portals
---
# Get Dynamics 365 for Customer Engagement data on the portal

One of the most distinguishing features surrounding the portal capabilities introduced in Dynamics 365 for Customer Engagement Online 2016 Update 1 is the ability to render&mdash;and allow the manipulation of&mdash;Dynamics 365 for Customer Engagement data on an internally facing or externally facing portal website. In this section, we will profile the entities that provide the foundation of this functionality and discuss the steps necessary to achieve it.

## Entity forms
An entity form record allows an administrator to render a Dynamics 365 for Customer Engagement form on a portal to edit, display, or capture a customizable set of data from its users. This is a relatively easy and straightforward process, and it eliminates the need for any code customizations that would have otherwise been required to achieve such a result before deploying a portal.

This functionality can be taken advantage of for many reasons and use cases, but a common example of how this can be leveraged would be to build a new form under the Lead entity within the Dynamics 365 for Customer Engagement **Customize the System** interface, then create an entity form record that can be surfaced on the portal to capture lead data through a contact page or some type of information request page. In fact, this specific example is used for the default **Contact Us** page within the portals available in the December 2016 Update for Dynamics 365 for Customer Engagement (online) release. Other examples include using an entity form for profile management or a single page survey.
...
For this demonstration, let’s consider a scenario in which Contoso, Inc. would like to create a custom entity in Dynamics 365 for Customer Engagement linked to a form rendered on their Customer Self-Service Portal that will allow users to submit suggestions for improvement that they would like to see the company introduce. The steps that the administrator at Contoso, Inc. would take to achieve this include the following:

1. In Dynamics 365 for Customer Engagement, create the custom entity through **Settings** > **Customizations** > **Customize the System** > **Entities**.  In this example, we’ll call the entity “Suggestion.” Set the entity to appear in the **Portals** navigation area, then save it.

2. In Dynamics 365 for Customer Engagement, create two custom fields underneath the Suggestion entity that pertain to the information that should be captured on the form.  In this example, we’ll create **SuggestionOrigin** (Option Set) and **SuggestionSummary** (Multiple Lines of Text) fields.  Both fields will be set to **Business Required**.

3. In Dynamics 365 for Customer Engagement, create a new main form underneath the Suggestion entity:
...
5. In the portal, create a new child page named **Suggestions**. Set its **Page Template** value to **Web Form**, set its **Entity Form** value to **Suggestion**, and add an introduction to the form. 
   > [!Note]
   > The child page can also be created via Dynamics 365 for Customer Engagement, depending on the user’s preference.
...
When a user visits the Contoso portal, navigates to this form, and enters the relevant data and submits it, a new record will automatically be generated in Dynamics 365 for Customer Engagement with the submitted information.
...
Entity form metadata records contain additional behavior modification logic to augment or override the functionality of form fields that you otherwise could not modify by using the native entity form editing capabilities in Dynamics 365 for Customer Engagement.
...
To add metadata records to an entity form, you can go to the **Entity Form Metadata Associated View** from the Dynamics 365 for Customer Engagement navigation menu, or you can create these records directly from the entity form itself by using the Entity Form Metadata subgrid toward the bottom of the record’s form.
...
## Entity lists
While an Entity Form record allows a portal administrator to expose a Dynamics 365 for Customer Engagement form to portal users, entity lists provide administrators with the ability to add a webpage to their portal that will render a list of Dynamics 365 for Customer Engagement records without the need for a developer to surface the view on the portal by using custom code.
...
Entity lists also support multiple views and actions. If more than one view has been specified in Dynamics 365 for Customer Engagement, a drop-down menu will be rendered to the end user that allows them to toggle between various views. Likewise, if actions such as **Edit** or **Delete** have been added as action buttons, users with the correct permissions will see these options rendered in the view.
...
1. In Dynamics 365 for Customer Engagement, go to **Settings** > **Customizations** > **Customize the System** > **Entities** > **Suggestions** > **Views** and edit the **Active Suggestions** view.  Add the columns that should be exposed on the portal to this view.
...
20. On the “Survey Step 2” Web Form Step record:
    1. Select the rightmost chevron in the main Dynamics 365 for Customer Engagement navigation and select **Metadata** to open the Web Form Metadata Associated View underneath the “Survey Step 2” record.
...
21. In Dynamics 365 for Customer Engagement, go to **Portals** > **Web Pages** and create a new Web Page named **Contoso Ltd. Survey**.  Set the **Website** to **Contoso Ltd.”**, the **Parent Page** to **Home**, the **Partial URL** to **contoso-ltd-survey**, the **PageTemplate** to **Full Page**, the **Publishing State** to **Published**, and the Web Form to the “Contoso Ltd. Survey” that was created earlier.  Save the record.

Granted, these steps are very complex and require several Dynamics 365 for Customer Engagement records to work in tandem with one another to achieve the result. However, after the process is complete, users can navigate to the page that you’ve created on the portal and fill out the survey as intended.
...
Each completed survey that is submitted will become a record within the **Portals** > **Survey** entity in Dynamics 365 for Customer Engagement, and this record will contain the contact who completed the web form along with all their responses for tabulation.

## Form elements rendering

Although most Dynamics 365 for Customer Engagement forms can be successfully rendered in a portal, note that some features aren't supported by the Dynamics 365 for Customer Engagement EntityFormView control that handles the actual rendering of the entity form.


|                                 Supported form elements                                 |                                Supported, but with caveats form elements                                 |                         Unsupported form elements                          |
|-----------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------|
| All Dynamics 365 for Customer Engagement field validation rules  |  All Dynamics 365 for Customer Engagement field types, except Party List fields   |         Form headers and footers won't be displayed in the portal          |
|                          Both single and multi-column layouts                           |                 Only “Webpage (HTML)” and image-related Web Resource types are supported                 |                                  iFrames                                   |
| Dynamics 365 for Customer Engagement label language translations |             Record filtering in a lookup field is supported, but only for N:1 relationships              | Dynamics 365 for Customer Engagement form scripting |
...
Finally, it’s worth mentioning that the Entity Form record in Dynamics 365 for Customer Engagement has an assortment of options that allow for it to be fine-tuned as needed, including a section for **Attach File** options.  If a form requires the attachment of files, select the **Attach File** check box and configure the **Attach File Storage Location** option for either **Dynamics 365 for Customer Engagement Note Attachment** or **[!include[Azure](../includes/pn-azure-shortest.md)] Blob Storage**.
