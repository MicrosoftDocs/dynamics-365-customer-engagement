...
> [!NOTE]
> If the portal cannot retrieve a content snippet because it can't connect to Dynamics 365 for Customer Engagement or if the snippet is not available in Dynamics 365 for Customer Engagement, an error message appears.
...
> [!NOTE]
> If you change the Dynamics 365 for Customer Engagement instance that your portal is connected to, diagnostic logging is disabled. You must enable diagnostic logging again.
...
Another scenario that often occurs while developing a portal is an error generated by custom plug-ins and business logic written in your Dynamics 365 for Customer Engagement organization. These errors can generally be accessed by [disabling custom errors](#disable-custom-error) or [enabling diagnostic logging](#enable-diagnostic-logging). However, in some cases, it is faster to display these errors directly on the portal to diagnose the issue faster. To do this, you can configure your portal to display custom plugin errors from Dynamics 365 for Customer Engagement on your portal screen.
