...
When a product bundle is added<!--Passive voice needed here, to avoid a misplaced modifier. --> to an opportunity, quote, order, or invoice, a required product in the bundle uses the bundle price for price calculations. When the bundle added to the opportunity, quote, order, or invoice includes optional products, the total price is calculated by adding the total price of the optional products to the price of the product bundle<!--Recommend keeping the term "product bundle," for consistency. If you use "bundle product," it needs to be "bundled product."-->. 

Let's understand this with an example. You have a product bundle priced at $500, with the following products included in it: 
<!--Suggest using dollar signs in the tables, just to distinguish the count from the amount. -->
...
Now, let's see how the calculation differs if the product bundle priced at $500 includes the following optional products:
