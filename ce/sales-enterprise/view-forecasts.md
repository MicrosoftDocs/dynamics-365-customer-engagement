...
4.	In the drop-down below the View drop-down, select a forecast definition to see the forecasts for it. In this drop-down list, you’ll see the definitions that you own or the definitions of the forecast hierarchy that you are a part of.
 
    ![Drop-down to select forecast definition](media/forecast-definition-drop-down.png "Drop-down to select forecast definition")

    The forecasts are grouped by the forecast period. For example, if you selected a forecast definition that has the **Forecast period** set to **Quarterly**, **Fiscal year** to **FY 19**, **Start quarter** to **Q1**, and **No. of forecasts** to **4**, the forecasts that are generated will be grouped by four quarters, FY 2019 Q1, FY 2019 Q2, FY 2019 Q3, and FY 2019 Q4. 
...
Forecasts are shown based on the sales team hierarchy. For example, if Tonya Cortez is a sales manager with 5 directs, and she is set as a manager in each of her directs’ user record, she will see a forecast for herself and her 5 directs. 

![Forecasts for a manager and their directs](media/forecast-for-team-hierarchy.png "Forecasts for a manager and their directs")

If Tonya reports in to Bert Hair, Bert will see his own forecasts along with Tonya's rolled up forecast. Bert will not see the forecasts of Tonya's reportees.


To see the forecast for everyone in the hierarchy, expand a specific forecast.

The parent row (for the manager) shows values rolled up from all the child records (for the directs and manager themselves if the manager is also owning some participating opportunities).
...
While reviewing the forecasts there can be a need to make adjustments without directly impacting the participating opportunities. For example, if the sales manager thinks that one of his directs Best case opportunities may not result in $60000, they can change the value in the forecast to a different value. When you save the forecast after making the adjustment, the changes are rolled up to the parent record.
...
-	The aggregation and rollups for the underlying data is recalculated
