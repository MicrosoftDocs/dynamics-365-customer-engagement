...
| Data type | Select the data type of the column that you want to display on the forecast grid. The supported values are currency, integer, and decimal. You can use integer or decimal data type for quantity-based forecast. By default, the value is selected as **Currency**. If you have multiple data types defined for your forecast such as, currency and integer, you can't activate a forecast and view data in Kanban. Similarly, if you have enabled the predictive forecasting, deal charts and trend flows will not be displayed. While publishing the forecast, you must select a primary data type as currency or quantity to display the data in kanban view, deal flows, and trend charts. To learn more, see [Resolve multiple data type columns issue]() |
...
| Parameter | Description |
|-----------|-------------|
| Data type	| Select the data type of the column that you want to display on the forecast grid. The supported values are currency, integer, and decimal. Depending on the type of value that you'll be uploading data through an Excel workbook. You can use integer or decimal data type for quantity-based forecast. By default, the value is selected as **Currency**. If you have multiple data types defined for your forecast such as, currency and integer, you can't activate a forecast and view data in Kanban. Similarly, if you have enabled the predictive forecasting, deal charts and trend flows will not be displayed. While publishing the forecast, you must select a primary data type as currency or quantity to display the data in kanban view, deal flows, and trend charts. To learn more, see [Resolve multiple data type columns issue]() |
| Description | Enter a description for the column. This description appears as a tooltip on the column header of the forecast grid, to help your users understand what the column contains. |
| Unique name | The unique name is used to identify the column to use when you're writing a formula for a **Calculated** column. |
...
| Parameter | Description |
|-----------|-------------|
| Data type	| Data type of the column that you want to display on the forecast grid. By default, the value is selected as **currency** and can't be changed. |
