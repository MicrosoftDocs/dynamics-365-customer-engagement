...
- Turn on/off cards
- Assign/remove roles to a card
- Set priority to a card
- Edit the flow of a card

## Turn on or off cards

Different users have different needs in using the cards, and you don’t want to show the card for some users at this point of time. Similarly, you don’t want to show certain cards in your organization or might not require to be displayed at this point of time. You can turn on or off cards depending on the requirement to show to specific user roles in the **Assistant** section of the **Sales AI** setup center.

When you turn off a card, the assistant feature disables the card for the security roles you have assigned the card to. However, the assistant feature doesn't disable other properties that are associated with the card. For card that you generated using Microsoft Flow, you can disable the card, but you can't delete the flow. Even after you disable the card, the flow remains active as other custom insight cards might use the Flow.

Go to **Assistant** tab and open a card from the list that you want to turn on/off. Select the **On**/**Off** toggle as required. In this example, we are turned on the **Upcoming Meeting (Exchange)opportunity** card.
    
![Turn card on or off](media/cc-edit-card-on-off-toggle.png "Turn card on or off")

### Turn on/off multiple cards

To turn on or off multiple cards, go to **Assistant** tab and select the cards that you want to turn on/off. Select the **Turn on cards** or **Turn off cards** as per your requirement.
...
### Turn on/off cards for a security role

If you want to turn off cards for a particular security role, go to **Assistant** tab and filter the cards based on the role. Choose the cards that you want to turn off for the selected role and select **Remove for this role**. The cards will not show for the role that you have selected.

In this example we have filtered the cards with the security role **Sales manager** and chosen the cards - **Card name placeholder**, **Opportunity at risk**, and **Stakeholder recommendations**. Select **Remove for this** role and the cards are turned off only for the **Sales manager** role. 
...
## Assign or remove roles to a card
...
1. Go to **Assistant** tab and select the card for which you want to add the security roles. In this example we have select **Opportunity Health Score Changed** card.
...
2. Select **Show by security role (preview)** section and the list of security roles that are available in your organization is displayed.
   ...
   In this example we have added **Marketing Manager** security role to the **Opportunity Health Score Changed** card.

   ![Security role added to card](media/cc-edit-card-assign-role-added-role.png "Security role added to card")

3. Close the card.
4. (Optional) To remove security roles, select the role to remove. In this example, we are removing **Marketing Manager** security role to the **Opportunity Health Score Changed** card.

   ![Remove security role for card](media/cc-edit-card-assign-role-remove-role.png "Remove security role for card")

## Set priority to a card

You can prioritize the cards that display in your organization. when you set a card as priority, the card gets displayed to the user on top.

When you open the **Assistant** tab, the list of cards that are defined for your organization is displayed. A check mark corresponding to the card specifies that the card is set as priority. In this example, **Opportunity at risk** card is set as high priority. This card will be promoted above other cards and displayed on the top of others.
    
![Card priority list view](media/cc-edit-card-assign-priority-list.png "Card priority list view")

1. Go to Assistant tab and open the card that you want to set as priority. Select **High Priority (preview)**. On this example, we are selecting, and prioritizing **Upcoming Meeting (Exchange)** card.
...
You can always edit the flow of the card when there is a business requirement for you to update the flow of the card to meet your organization’s necessity. You can add or update conditions, steps, and update the properties of a condition. 

> [!NOTE]
> The **Edit in Flow** option appears only for the cards that are created in the Microsoft Flow.

1. Go to **Assistant** tab and select the card for which you want to change the flow. In this example we have select the **Opportunity Health Score Changed** card.

   ![Open card to edit in flow](media/cc-edit-flow.png "Open card to edit in flow")
 
2. Select **Edit in Flow** and the flow opens in a tab to editing.
