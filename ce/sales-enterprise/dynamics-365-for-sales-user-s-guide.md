---
title: "Dynamics 365 for Sales - User&#39;s Guide | MicrosoftDocs"
ms.custom: ""
ms.date: 08/31/2017
ms.reviewer: ""
ms.service: "crm-online"
ms.suite: ""
ms.tgt_pltfrm: ""
ms.topic: "get-started-article"
applies_to: 
  - "Dynamics 365 (online)"
  - "Dynamics 365 Version 9.x"
ms.assetid: b16a9d08-e24a-4b60-9dcc-6803cea4882e
caps.latest.revision: 5
author: "brycho"
ms.author: "brycho"
manager: "amyla"
---
# Dynamics 365 for Sales - User&#39;s Guide
[!INCLUDE[pn_dyn_365_sales](../includes/pn-dyn-365-sales.md)], Business edition helps you zero in on the right leads, contacts, and opportunities as you build out your team's sales pipeline. Built-in processes guide you from one stage of the sales process to the next, and  easy-to-use dashboards help you set goals and provide actionable insights to help you close more deals.  
  
<a name="heroArea"></a> ![](media/Convert-Leads-to-Opportunities-Resized.png "Convert Leads to Opportunities: Hero image")

## Overview  
 [Understanding how data is organized](../basics/understanding-how-data-organized-microsoft-dynamics-365.md)  
  
 [What are business processes?](../basics/what-are-business-processes.md)  
  
## Get started  
 [Find your way around the apps](../basics/find-your-way-around-dynamics-365-or-dynamics-crm-earlier-versions.md)  
  
 [Start your day with a dashboard](../basics/start-your-day-with-a-dashboard.md)  
  
 [Stay up-to-date with customer news with the activity feed](../basics/stay-up-date-with-customer-news-with-activity-feed.md)  
  
 [Work with activities](../basics/work-with-activities.md)  
  
## Keep track of customers  
 [Create or edit an account](../basics/create-edit-account.md)  
  
 [Create or edit a contact](../basics/create-edit-contact.md)  
  
 [Deactivate or activate an account or contact](../basics/deactivate-activate-account-contact.md)  
  
## Manage the sales process  
 [Nurture sales from lead to order](../sales-enterprise/nurture-sales-from-lead-order-sales.md)  
  
 [Create or edit a lead](../sales-enterprise/create-edit-lead-sales.md)  
  
 [Qualify a lead and convert it to an opportunity](../sales-enterprise/qualify-lead-convert-opportunity-sales.md)  
  
 [What if you're no longer working a lead?](../sales-enterprise/what-if-no-longer-working-lead-sales.md)  
  
 [Re-open a lead](../sales-enterprise/re-open-lead-sales.md)  
  
 [Create or edit an opportunity](../sales-enterprise/create-edit-opportunity-sales.md)  
  
 [Close an opportunity as won or lost](../sales-enterprise/close-opportunity-won-lost-sales.md)  
  
## Create and work with collateral  
 [Create or edit a quote](../sales-enterprise/create-edit-quote-sales.md)  
  
 [Create or edit an order](../sales-enterprise/create-edit-order-sales.md)  
  
 [Create or edit an invoice](../sales-enterprise/create-edit-invoice-sales.md)  
  
 [Create a product](../sales-enterprise/create-product-sales.md)  
  
## Create a marketing campaign  
 [Create a marketing list using in-app marketing](../sales-enterprise/create-marketing-list-using-app-marketing-sales.md)  
  
 [Create a quick campaign using in-app marketing](../sales-enterprise/create-quick-campaign-using-app-marketing-sales.md)  
  
## Create and manage goals  
 [Create or edit a goal](../sales-enterprise/create-edit-goal-sales.md)  
  
 [Create or edit a goal rollup query](../sales-enterprise/create-edit-goal-rollup-query-sales.md)  
  
## Reports, calendar, and printing  
 [Run a report](../basics/run-report.md)  
  
 [Add or edit Power BI visualizations on your dashboard](../basics/add-edit-power-bi-visualizations-dashboard.md)  
  
 [Use the workplace calendar](../basics/use-workplace-calendar.md)  
  
 [Print leads, quotes, and other records](../basics/print-leads-quotes-other-records.md)  
  
## Import and export data  
 [Import contacts](../basics/import-contacts.md)  
  
 [Import accounts, leads, or other data](../basics/import-accounts-leads-other-data.md)  
  
 [Export data to Excel](../basics/export-data-excel.md)  

<!-- not sure if we need this anymore. Legacy is suspicious>  
## Set personal options  
 [Set personal options - legacy](Set%20personal%20options%20-%20legacy.md)  
-->
  
## Go mobile  
 [Dynamics 365 for Phones and Tablets User's Guide](../mobile-app/dynamics-365-phones-tablets-users-guide.md)  
  
## Work with Dynamics 365 data from Microsoft Outlook  
 [Dynamics 365 App for Outlook User's Guide](../outlook-app/dynamics-365-app-outlook-user-s-guide.md)  
  
## Get better sales intelligence  
 [Add Social Engagement visuals to a dashboard or account](../social-engagement/add-social-engagement-visuals-dashboard-account.md)  

 [Boost sales with Versium Predict](../versium-predict/versium-predict.md)  

 [Get sales Insights, powered by InsideView](../insights-insideview/insights-powered-by-insideview.md)  
  
## Security  
 [Assign a record to a user or team](../basics/assign-record-user-team.md)  
  
 [View your user profile](../basics/view-your-user-profile.md)