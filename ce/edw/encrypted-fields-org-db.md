...
3.  In the Data Encryption dialog box, notice that the **Encryption status** label indicates that data encryption is **Inactive**. To activate data encryption:  
    ...
    > [!WARNING]
    >  If you cannot provide the encryption key, data encryption cannot be activated and the associated data will become inaccessible. Features in Microsoft Dynamics 365 that use the encrypted data, such as Server-side Synchronization, will no longer work. Notice that you cannot delete and re-create the encrypted data.  
