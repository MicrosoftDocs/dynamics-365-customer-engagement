...
The **Journey** tab displays information on the path (recent actions) the customer took just before initiating a conversation. The information enables the agents to understand the custmer intent and provide a personalized service with fast resolution for improved customer satisfaction. The **Journey** tab displays the information in the reverse chronological order, and is categorized into the following type of actions:
...
To configure the journey tab, see [Enable visitor journey](../../administrator/journey.md).
