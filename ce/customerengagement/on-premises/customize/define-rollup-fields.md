...
Rollup fields are designed to help users obtain insights into data by monitoring key business metrics. A rollup field contains an aggregate value computed over the records related to a specified record, such as open opportunities of an account. Also, you’ll be able to aggregate data from the activities directly related to a record, such as emails and appointments, and activities indirectly related to a record via the Activity Party entity. When you include indirectly related activities, all activities related to the specified record will be considered, regardless of participation type. For example, an Account record can appear in the To, From, or CC fields for an email activity. However, activities associated with other related entities, such as those related to an Opportunity that is associated with an Account, will not be included in the aggregation for the Account’s rollup field. In more complex scenarios, you can aggregate data over the hierarchy of records. As an administrator or customizer, you can define rollup fields by using the feature without needing a developer to write code. 
