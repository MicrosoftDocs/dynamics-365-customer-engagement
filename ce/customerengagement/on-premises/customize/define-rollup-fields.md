...
Rollup fields are designed to help users obtain insights into data by monitoring key business metrics. A rollup field contains an aggregate value computed over the records related to a specified record, such as open opportunities of an account. Also, you’ll be able to aggregate data from the activities directly related to a record, such as emails and appointments, and activities indirectly related to a record via the Activity Party entity. When you choose to include indirectly related activities, all related activities will be considered regardless of participation type. For example, an Account record can appear in To/From/CC fields for an email activity. However, it will not consider related activities of other related entities. For instance, if Opportunity is a related entity of Account, a rollup field on Account entity aggregating on activities will not include Activities related to the Opportunity entity. In more complex scenarios, you can aggregate data over the hierarchy of records. As an administrator or customizer, you can define rollup fields by using the feature without needing a developer to write code.  
