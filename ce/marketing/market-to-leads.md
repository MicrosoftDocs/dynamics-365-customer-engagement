...
- By conceptualizing leads as _expression of interest_, you can ensure that they are first qualified from a marketing perspective, and then from sales perspective. When a salesperson enters a lead, then the system assumes that the sales team has already taken over the responsibility to engage with that lead, just as they would with an already marketing-qualified lead.
...
1. Go to the **Definition** tab to setup your query.
...
1. Set up a query to find the leads you need. In this example, we'll look for leads collected at a recent conference, which were all assigned a **Topic** of "Contoso Ltd Conference".  
...
1. Set the new relation to **All\*** to find all contact that relate to the found leads.  
...
Also, any user can manually select or create a contact for a lead by using the using the **Inquiry** stage of the **Lead to opportunity marketing sales process** business process on the lead record.
...
- Contact list views that are pre-configured to show or hide marketing-only contacts for various audiences as needed (for example, a marketing view that shows all contacts, and a sales view that hides marketing-only contacts).
