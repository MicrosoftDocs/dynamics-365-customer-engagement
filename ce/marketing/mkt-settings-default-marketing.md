...
Normally, Dynamics 365 Marketing deduplicates outgoing marketing email messages to ensure that each message is sent just once to each unique email address. To sent only just once, Dynamics 365 Marketing has a duplication detection process. This duplication detection is done by storing a key value which includes a contact's email address coupled with the information about the customer journey and the email tile. That means that if more than one contact record in the target segment has the same email address, only one of those contacts will receive the message.
Duplicate email addresses probably indicate that the same person is represented by two different records in your database (for example, because they registered at different times using two different first-name variants, such as "Bob" and "Robert"), so this is the desired behavior.
