...
- **Handling unresolved profile**: This option is only available for journeys that target Customer Insights profiles. It takes time to create a full Customer Insights profile. If the full profile isn’t available at the time the person triggers the journey, they can either start the journey immediately using defaults for any missing profile data or wait for the full profile to be available before starting the journey. To successfully communicate with someone without a profile the event trigger must specify email (contactpoint_email) or phone (contactpoint_phone) attributes. See [Create a custom event trigger](real-time-marketing-custom-events.md)
