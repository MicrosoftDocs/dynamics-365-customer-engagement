...
- **Scope**: All segments will only include contacts that belong to the same business unit as the segment owner or (if business-unit scoping is enabled) the scope selected here. Most standard users can only use a scope that matches their own business unit, but privileged users can select a reduced scope if they wish. More information: [Use business units to control access to Marketing records](business-units.md)
