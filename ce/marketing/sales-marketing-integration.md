...
Read this topic to learn how marketers and salespeople can collaborate through dedicated, optimized sales and marketing apps running on top of a unified Dynamics 365 platform.
...
1. The marketer [creates a customer journey](customer-journeys-create-automated-campaigns.md) to implement an awareness campaign that includes the following:
    ...
    - A [segment group](customer-journey-tiles-reference.md#segment) that includes both the segment designed by the marketer and a static segment where salespeople can [manually add promising contacts](manage-segments-from-contacts.md) based on their knowledge about those contacts.
    - An [email tile](customer-journey-tiles-reference.md#email) that delivers the promotional email to each member of the target segment.
    - A [trigger](customer-journey-tiles-reference.md#trigger) that sorts contacts based on whether they submitted the landing page to receive the white paper.
    - After the trigger comes a second email that will deliver the link to download the paper to contacts who submitted the form. Following this is a [create-lead tile](customer-journey-tiles-reference.md#create-lead) that generates a lead for each of those contacts.
...
1. Depending on how a customer flows through the nurturing journey, some branches could end with an [activity tile](customer-journey-tiles-reference.md#activity) that assigns an activity, such as a phone call, to a salesperson. This gives the salesperson an opportunity to engage with especially promising contacts right away, even though they are still in the marketing funnel. Or you might instead use a [launch-workflow tile](customer-journey-tiles-reference.md#launch-workflow) to trigger an even more complex set of events when a contact lands at a certain destination in the journey.
