...
| Payment gateway |  [Build a gateway page](event-payment-gateway.md) using native Dynamics 365 Portal entities and customize it according to instructions from your third-party payment provider  | [Build a gateway page](event-payment-gateway.md) using native Dynamics 365 Portal entities and customize it according to instructions from your third-party payment provider | [Customize the Angular app](developer/event-management-web-application.md) according to instructions from your third-party payment provider |
| Registration&nbsp;account and sign-in | Implemented using native Dynamics 365 Portal features; works out-of-the-box | Implemented using native Dynamics 365 Portal features; works out-of-the-box | You must [integrate the event website with Azure Active Directory (AAD)](developer/event-management-web-application.md) to enable registration accounts |
| The **Payment gateway** setting of the event record | Use this setting to select a Dynamics 365 Portal page to use as the payment gateway for each event | Use this setting to select a Dynamics 365 Portal page to use as the payment gateway for each event | This setting has no effect for externally hosted event websites |
