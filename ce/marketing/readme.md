...
- While creating a segment, if a query is built using an entity of the type "systemuser" (for example, entity "owners"), the segment would not be created. As a workaround, the guid of that particular entity (retrieved from the url of the entity) can be leveraged to create such a segment. <!--- 1335786 -->
