...
- When using Assist Edit to leverage a contact and its owner relationship, for creating dynamic text, the relationship rendered is incorrect. Instead use the following:  
  -If the contact's owner is a user, use **contact.contact_systemuser_owninguser.<fieldname>** (e.g. contact.contact_systemuser_owninguser.firstname )  
  -If the contact's owner is a team, use **contact.contact_team_owningteam.<fieldname>** (e.g. contact.contact_team_owningteam.firstname )  
   Please ensure that the relevant entity is synced to Customer Insights.  <!-- 1380000 -->
