...
- If you stop a live customer journey, and then add contacts to the target segment and restart the journey, these added contacts might receive emails sent by this journey more than once. <!---1196442 - November Release-->
- In a non-English org, when creating a customer journey template using "Save As Template" functionality, the template gets saved with the "English" set as the language. To fix this, navigate to Marketing > Customer journey templates and select that particular template, and update the language to the correct value. While creating a new customer journey from a template, you can use the language dropdown on the template gallery to switch between templates in different languages. <!--- 1287079 - November 2018 Release -->
...
- On a static segment definition after adding filters and clicking on the "apply" button, the segment defintion may stop showing members. You can save the segment, navigate to the segment list and navigate back into the segment to see the list of members and continue curating it. <!--- 1282842 -->
...
- In certian orgs, test sending an email throws an error and does not send the email. <!--- 1267485 - November-->

### Fixed issues

- When creating a new Keyword, using the "Save and Close" functionality works correctly and navigates to the Keywords grid.
  <del>When you save a new keyword for templates or files from a quick-create form, the keyword grid isn't displayed, but the record is created successfully. <!--- 1033440 - October Release --></del>
...
- Marketing pages from the sample data might not go live for some organizations (especially on Italian or Japanese localizations). But can still create new pages and go live with them. <!--- 1156824 - April Release -->
...
- While creating a lead scoring model and adding a condition, the "Email sent" condition is not available from the list of options. Instead use the "Sent" condition. <!--- 1289271 -->
...
- Events now have sample data.
 <del>Sample data is no longer available for events. We expect to provide it again in a future update.</del>

## Videos

- When a video is embedded in a marketing page, the thumbnail is shown in the designer pane, and not the embedded video iframe. On the live marketing page however, the embedded video iframe shows correctly. <!--- Provided as a tip -->
- When a video is embedded in a marketing page or email, and the content is previewed in the application after saving it, the embedded thumbnail is not visible. The video is correctly embedded and shows up on the email sent and live marketing page. <!--- 1277775 - November 2018 -->
...
- The social listening control may not show data for certian orgs. In order to mitigate this, please open a support ticket. <!--- This is for the UCI FCB that's required to be switched on, and required a platform upgrade. We can also manually flip this FCB on an org by org basis by updating a DB value -->
