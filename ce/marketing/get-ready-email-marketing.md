...
- **Authenticate your messages and sending IP**  
    Several standards are in place to help ensure that sending IP addresses really do represent the organizations they claim to, and that messages coming from those IPs are authentic and untampered with. These include *Sender Policy Framework* (SPF), *DomainKeys Identified Mail* (DKIM), and *Domain-based Message Authentication, Reporting and Conformance* (DMARC). [!INCLUDE[cc-microsoft](../includes/cc-microsoft.md)] does what it can to maintain a good sender reputation and to sign and authenticate messages sent from our servers, but you can also improve deliverability by associating your [!INCLUDE[pn-microsoftcrm](../includes/pn-dynamics-365.md)] sending IPs with your organization's domain name in the DNS system (as described in the next section).
