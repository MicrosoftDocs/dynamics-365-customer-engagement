...
1. Make the following settings, as needed:
    ...
    - **Default for owning business unit**: This setting is only available when [business units](business-units.md) scoping is on. Set this to **Yes** to make the current content-settings record the default for all new customer journeys within business unit. There must always be exactly one default content setting for the business unit scope; if you change the default, the existing default will automatically be changed to **No**.
