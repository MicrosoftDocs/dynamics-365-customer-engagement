...
- If *&lt;value1&gt;* is a Boolean field (also known as a *two options* field in Dynamics 365), then don't include an *&lt;operator&gt;* or *&lt;value2&gt;*. Boolean fields always have a value of either true or false, so you should just use them directly to establish the condition. For example:<br>`{{#if (contact.is_vip_member)}} ... {{/if}}`.
