---
title: "Preview: Use custom event trigger actions in real-time marketing journeys (Dynamics 365 Marketing) | Microsoft Docs"
description: "Learn how to use custom event trigger actions in real-time marketing journeys in Dynamics 365 Marketing."
ms.date: 02/07/2022
...
# Preview: Use custom event trigger actions in real-time marketing journeys
...
Here are some examples of where you might use custom event trigger actions:
...
*Customer 2: Enters the journey as a new customer and proceeds down the **Yes** branch. In the **Yes** branch, they reach a custom event trigger that you've connected to a Power Automate flow. Because the customer has never made a prior purchase, you want to prefill relevant information about the customer using the purchase data. The flow assigns a rewards number to the customer if there’s a valid phone number provided at the time of registration.*

Custom event actions are composed of three steps: select a custom event, understand the selected event usage, and map attributes.
...
> [!NOTE]
> Power Automate flows that reference the custom event trigger are not tracked or displayed here.
