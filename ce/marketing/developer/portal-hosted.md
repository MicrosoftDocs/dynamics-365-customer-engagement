...
To make your customizations visible in your event portal you need to update the web files in Portals. This can be done automatically by using the provided script called `DeployToDynamics365Instance.ps1`, or manually by updating the attachments of all web files.

### Automatic Deployment (preferred)
To deploy the customized Angular application, run the **PowerShell** script `DeployToDynamics365Instance.ps1` located under the **Scripts** directory. The script builds the Angular application, prepares the output files to be suitable for hosting under the Dynamics 365 Portal instance. It takes the localization files from the `Localization` folder and prepare them for hosting.

After all files are built it asks you to log into Dynamics 365 instance that you want to use to host your application. After you log in, it pushes the files to the instance. 

That's it. You're changes should be live. If you struggle to see your changes please clear your browser cache and restart Portals.

> [!NOTE]
> If you can't run the Powershell script because of your execution policy then you might need to [disable / bypass the execution policy](https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.security/set-executionpolicy?view=powershell-6).


### Manual Deployment

To manually replace the files in Dynamics 365, follow the steps below (step 3 - 5 must be repeated for each file that has changed in your `dist` folder):
...
You can find a mapping between generated Angular files and Portal web files in the `DeployToDynamics365Instance.ps1` script if you struggle to understand which file in `dist/ClientApp` folder belongs to which web file.
