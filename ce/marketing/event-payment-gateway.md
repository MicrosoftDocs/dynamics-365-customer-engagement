...
- If you are hosting the event website on a Dynamics 365 for Customer Engagement portal, then see  [Create a payment gateway when hosting on the Dynamics 365 portal](#portal) for instructions.
...
1. Sign in to Dynamics 365 and go to the custom app by choosing **Dynamics 365—custom** from the app selector.  
...
Your back-end service must authenticate against your Dynamics 365 for Customer Engagement instance to enable the service to execute the custom actions needed to finalize the workflow. More information:
...
After your back-end solution has verified payment, it must invoke the `msevtmgt_FinalizeExternalRegistrationRequest` (or `adx_FinalizeExternalRegistrationRequest` if you are using a version prior to 1.13 (April release 2019)) custom action against your Dynamics 365 for Customer Engagement instance. This custom action requires the following input parameters:
...
- `UserId`: Identifies the contact who made the purchase. This is the ID for the contact record in Dynamics 365 for Customer Engagement.
