...
- **Solution-only license**: A solution-only license includes all of the entities and solutions for Dynamics 365 Marketing, but doesn't include any of the marketing services, which means that many features, including email delivery and insights, won't be available. This license is intended for use on development instances for testing customizations and compatibility issues. This type of license is sometimes included when you purchase a full license (depending on your selected licensing option).
...
1. Find and select an unconfigured Dynamics 365 Marketing application in the list on the **Applications** tab. Then choose **Manage** from the side panel. You may have more than one type of application available, so be sure to pick the one that meets your present needs. Both types of applications present the same setup wizard, as described in this procedure. They are identified as follows:
    ...
    - **Dynamics 365 Marketing - Solution Only**: This application provides the Dynamics 365 Marketing solutions, including all database entities, but not the services. Many features, including email delivery and insights, won't be available. It is intended for use during development and testing only.
