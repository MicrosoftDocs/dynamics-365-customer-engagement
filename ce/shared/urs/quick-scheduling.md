...
- When there are no search start and search end dates used on the resource requirement, requirement group, or the schedulable entity, then the search window defaults to 60 days from today, and resource availability is shown for the next 60 days. When there is only Search start date populated, then search window defaults from the Search start date to the 60 day window. 
