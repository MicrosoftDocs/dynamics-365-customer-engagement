...
- The Resource available timeslots shown on the Quick Book, would respect the ‘Book based on’ setting on the default Schedule Board settings and will show the resource availability based on the value chosen for **Book based on**,ie.,**Estimated arrival time** or **Start Time**. This behavior is aligned to how Schedule Assistant also shows the resource availability based on **Book based on** setting.
