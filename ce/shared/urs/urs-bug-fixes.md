...
- Fixed an issue where a new Booking is created from the Booking form directly with a **Booking rule** enabled, the RequirementId is not passed via the bookins rule. 
...
- Booking a multi-day requirement stuck in an loop for long time when using Booking method as **Remaining Requirement** with **Double-book if needed** selected. This issue is now fixed. 
- When the user selects a suggested timeslot on the quick book panel, the selection was not highlighted, this bug is now fixed. 
