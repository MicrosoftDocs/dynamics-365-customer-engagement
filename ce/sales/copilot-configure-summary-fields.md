...
- You must have an administrator security role, such as System Administrator. [Learn more about predefined security roles](/power-platform/admin/database-security#predefined-security-roles).
- Copilot must be turned on in your environment. If the Copilot settings aren't available, ask your admin to [turn on and set up Copilot](enable-setup-copilot.md).

## Configure summary and recent changes fields

> [!NOTE]
> For leads processed by the [Sales Qualification Agent](sales-qualification-agent.md), Copilot always displays the summary generated by the agent, irrespective of the fields you configured for lead summary.   
...
1. Select the tab for the type of fields you want to configure:
    - **Summary** tab to configure fields for summaries
    - **Recent changes** tab to configure fields for the recent changes list
