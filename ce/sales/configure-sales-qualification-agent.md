...
Question: Do we need this for public preview?
...
1. In the **Research** section, specify the research sources that the agent should use to gather information about leads. By default, the following sources are configured, which can't be edited:
   ...
   | Source | Description |
   |-------------------------|-------------------------|
   | Default basic research | Includes insights gathered from Copilot, who knows whom, related opportunities, and so on. |
   | Default web research | Includes insights from predefined web sources to gather authentic information about the company's business, finances, and news. |
...
1. (Optional) [Add a custom criteria for ideal customer profile](#add-custom-criteria-for-ideal-customer-profile).
