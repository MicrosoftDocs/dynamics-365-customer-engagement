...
| Lead (lead) | **User-level**: Read, Write, Append, Append To |
...
| Always Run SQA Segment (msdyn_alwaysrunsqasegment) | **Global-level**: Read |
...
**Permissions required for assignment rule seller/team picker**

Ensure that the following permissions are granted to the custom security role to allow users with that role to be selectable in the seller/team dropdown when [configuring assignment rules](configure-sqa-assignment-rules.md) for the Sales Qualification Agent: 

- Lead: Write (User-level)
- Lead: Append (User-level)
- Sales Agent Handover: Read (Global-level)
- Sales Agent Handover: Delete (User-level)
- Sales Agent Handover: Share (User-level)
- Engagement and Readiness Agent Result V2: Write (User-level)
- SalesAgentProfile: Read (Global-level)
- Sales agent run: Read (Global-level)
- SalesAgentConfigurationv2: Read (Global-level)
...
2. Depending on the mode you want to deploy, complete the following configuration steps to set up the agent user:
...
3. [Configure general information](sales-qualification-agent-general-settings.md) for agent such as profile, company info, and products.  
4. [Configure selection criteria](sales-qualification-agent-selection-criteria.md) for the agent process the leads.  
5. [Configure email instructions, define custom outreach scenario, and specify custom email field](sales-qualification-agent-email-instruction-validation.md) for the agent.
6. [Configure handoff criteria](configure-sales-qualification-agent-handoff-criteria.md) for the agent to identify leads that are a good fit for further engagement.  
7. [Configure assignment rules](configure-sqa-assignment-rules.md) for the agent to assign processed leads to the appropriate sellers.  
8. [Configure knowledge sources](configure-sqa-knowledge-source.md) for company and competitor insights, outreach, and follow-up emails.
9. [Configure connected skills](sales-qualification-agent-general-settings.md) to enable the Sales Qualification Agent to surface action recommendations in the **Suggested actions** carousel.
10. (**Research and engage mode**) [Run simulation to review the agent's outreach emails](run-simulation-sqa-outreach-email.md) are customized and relevant to the leads.  
11. After you are satisfied with the configuration, [start the agent](start-sales-qualification-agent.md).
12. (Optional) [Test the Sales Qualification Agent](test-sales-qualification-agent-research-engage.md).
...
## Manage views in custom model-driven apps

If your sellers use a custom model-driven app instead of the Sales Hub, you need to ensure that the required Sales Qualification Agent views are added to the custom app for sellers to see AI-generated insights and use agent capabilities. Learn more in [Use Sales Qualification Agent in custom model-driven apps](sqa-custom-model-driven-app.md).
