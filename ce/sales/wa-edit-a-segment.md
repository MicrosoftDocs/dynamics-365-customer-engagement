...
After you [create a segment](./wa-create-and-activate-a-segment.md) and use it for a while, you might find that you need to tweak it to get better results. To edit a segment, you can either create a copy or make changes to the original directly. You can then view or modify the following characteristics of the segment: 
...
- Segmented records, or the records that are added to the segment: View
...
1. Select the vertical ellipsis beside the segment name, and then select **Create a copy**.
...
1. Make the necessary changes in the following tabs:
    ...
    - **Segmented *record type***: You can't change anything here, but you can view the records that you added to the segment. It's a good way to confirm the segment is defined correctly.
...
> [!NOTE]
> When you update the primary entity's attributes (such as lead or opportunity) that you add to conditions, all the segments related to that primary entity run again.
