...
1. Under the **Assign these leads to sellers or a team** section, select one of the following options in the dropdown list.
    ...
    | Queue | To assign leads to queues that matches specific attributes, select **Queues with matching attributes** and define the conditions through the condition builder.<p>**Note**: You can have a maximum of five conditions at any time, and only one row can be defined under a related entity condition.</p><p>Assign leads to a specific queue, select **Specific queue**. The lead is available for all members of the queue that you select. The queue must be enabled and defined in your organization. [Learn more about how to enable queue for an entity.](/customer-service/enable-entities-for-queues.md#enable-a-table-for-queues-)</p><p>If you select this option, the **Distribute leads by** section isn't shown, because leads are assigned to teams. |
