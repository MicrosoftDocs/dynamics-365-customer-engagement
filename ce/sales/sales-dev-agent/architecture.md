---
title: Sales Development agent architecture
description: This document describes the architecture of the Sales Development agent.
ms.date: 08/19/2026
ms.topic: overview
author: sbmjais
ms.author: shjais
...
:::image type="content" source="media/architecture.png" alt-text="Screenshot of a campaign summary report file.":::
...
End users interact with the Sales Development agent through familiar Microsoft applications such as Teams, Outlook, Word, Excel, and PowerPoint. Instead of being introduced as a separate tool, the agent appears as a contextual digital worker within day-to-day sales workflows.
...
This experience is designed for low friction and high trust. Users don't need to manage backend services, identity flows, or data integrations. They can focus on sales work while using the agent as a productivity accelerator in Microsoft 365.
...
Security and compliance policies are enforced consistently across all experiences. This approach helps ensure the agent aligns with enterprise requirements such as conditional access, data loss prevention, retention, and auditability.

## Agent runtime

The Agent runtime layer defines how agents execute, govern, and scale across Microsoft 365. It includes both the shared enterprise platform (Agent 365) and the domain-specific implementation (Sales Development agent).
...
The registry defines which agents, skills, and tools are available. You can invoke only approved and versioned components, which helps prevent unauthorized expansion of capabilities and uncontrolled behavior.

#### Access control

Agent 365 enforces identity-based access through Entra ID. All agent actions execute in the context of a specific user and tenant, supporting least-privilege operation and helping prevent cross-tenant or cross-user data leakage.
...
By running the agent inside this compliant container, you get enterprise-grade protections by default. You don't need to implement custom security controls for each integration.
...
Sales Development agent doesn't bypass enterprise controls to access external data. Instead:
