---
title: Outreach email architecture
description: This document describes the architecture of the Sales Development agent for outreach email generation and delivery.
ms.date: 08/19/2026
ms.topic: overview
author: sbmjais
ms.author: shjais
...
:::image type="content" source="media/architecture-outreach-email.png" alt-text="Screenshot of a campaign summary report file.":::
...
2. Sales Development agent initializes the outreach workflow
	...
	This container is a specialized runtime and isn't exposed through the Power Platform admin center.
...
3. Prospect research for each prospect
	...
	This enrichment step happens internally, even if it's not shown in the diagram. It directly informs the agent's reasoning and content generation. CRM data isn't required at this stage. If authorized CRM data is available, the agent can use it as an additional signal, but the workflow doesn't depend on it.
...
6. State tracking and ongoing execution
	...
	- Which prospects you contact
	- Which emails you send
...
8. Continuous security and compliance enforcement
	...
	There's no phase of the workflow in which security or compliance is bypassed or deferred.
...
- **Configuration approval**: The agent doesn't begin outreach until the hiring manager explicitly approves it.
...
- **Hot leads and escalation handoff**: When a prospect is identified as a hot lead or requires escalation, the agent notifies the hiring manager or assigned seller and loops them into the email thread. The agent doesn't continue autonomous engagement on escalated threads.
