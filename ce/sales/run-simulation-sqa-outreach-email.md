...
1. Go to the **Simulation** section and then select **Outreach emails**.  
    ...
    > [!NOTE]
    > The **Outreach emails** option is enabled only if you have leads that match the configured selection criteria and all the agent configurations are completed and marked with a green checkmark. Learn more in [Why can't the simulation find any leads?](sales-qualification-agent-faq.md#no-leads-for-simulation).
