...
# Create query to view sequence in entity record grid
...
2.	Create a new view for all the entities for which sequences view is required. The entity must be enabled for Sales Accelerator in the **Setup** page.   
...
5.	Choose **Entities** > **Add Existing** > **Entity** > find and add the entities for which view is being added (add the views created in step 2).   
...
15.	For each **View** added in step 2, locate its ```<savedquery>``` and add the following to its ```fetchxml```.
    ```xml
...
17.	Save the file.
18.	Zip the folder.
19.	Open a model-driven app in Dynamics 365.
