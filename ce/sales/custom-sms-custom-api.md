...
- The response is a serialized JSON with the following contract:
  ...
  - **ChannelDefinitionId**: GUID – The channel definition ID.
  - **MessageId**: string – (Optional) The message ID. Optional; it might exist in the API response of a provider.
...
If your channel supports message delivery reports or statuses, define a [delivery report custom API](/power-apps/developer/data-platform/custom-api). The provider calls your API, and it must call the base solution's **msdyn_D365ChannelsNotification** API, which returns the information to Dataverse. Sales chat editor provides access to this information.
...
If your channel supports inbound requests, you need to define an [inbound custom API](/power-apps/developer/data-platform/custom-api). The provider calls your API, and it must call the base solution's **msdyn_D365ChannelsInbound** API, which returns the information to Dataverse.
