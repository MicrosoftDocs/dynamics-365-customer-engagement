...
## Preventing Double Counting and Other Erroneous Results  
 Queries are effective in filtering the results of a rollup. However, if not used carefully, they can introduce the "double counting" or other erroneous results. The following examples demonstrate how queries might contribute in unwanted results:  
  
- You're tracking the sales orders for a particular salesperson. However, the rollup didn't return any sales orders. This scenario can happen if the query that you used filtered out the territories where the salesperson has customers.  
...
- Don't use a query if you aren't sure it will provide the results that you expect.  
