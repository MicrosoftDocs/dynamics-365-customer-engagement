---
title: Connect Dynamics 365 CX MCP Server - Sales through Agent 365 Tooling Gateway
description: Learn how to connect Dynamics 365 CX MCP Server - Sales to Copilot Studio and supported MCP clients through the Agent 365 Tooling Gateway.
author: lavanyakr01 
ms.author: lavanyakr
...
# Connect Dynamics 365 CX MCP Server - Sales through Agent 365 Tooling Gateway 
...
```http
GET https://agent365.svc.cloud.microsoft/.well-known/oauth-protected-resource/mcp/environments/<environment-id>/servers/mcp_D365CX_Sales
```
...
```json
{
  "resource_name": "mcp_D365CX_Sales",
  "resource": "https://agent365.svc.cloud.microsoft/mcp/environments/<environment-id>/servers/mcp_D365CX_Sales",
  "authorization_servers": [
    "https://login.microsoftonline.com/organizations/v2.0"
  ],
  "scopes_supported": [
    "https://agent365.svc.cloud.microsoft/mcp/environments/<environment-id>/servers/mcp_D365CX_Sales/.default",
    "openid",
    "profile",
    "offline_access"
  ],
...
- The token audience is tooling gateway, not the Dynamics 365 CX MCP Server - Sales directly.
...
- System Administrator role to configure the MCP server.  
...
Agent 365 Tooling Gateway uses a Microsoft Entra app ID. Make note of the following app ID.
...
```http
https://agent365.svc.cloud.microsoft/mcp/environments/<environment-id>/servers/mcp_D365CX_Sales
```
...
The Agent 365 Tooling Gateway app is a Microsoft first-party app registration. Before a client app can request a token for it, the tooling gateway service principal must exist in the customer tenant and the delegated permission must have admin consent.
...
1. Create a Microsoft Entra app registration by following the steps in [Register an application](/entra/identity-platform/quickstart-register-app#register-an-application). Ensure you use a single-tenant app registration.
...
1. Add the delegated permissions that the Agent 365 Tooling Gateway app exposes for `McpServers.D365Service.All` and `McpServers.D365Sales.All` 
...
1. Follow the steps in [Add tools and resources from a Model Context Protocol (MCP) server to your agent](/microsoft-copilot-studio/mcp-add-components-to-agent) to create a new MCP server and provide the following details:
   ...
   | Server description | Describe the available capabilities so the orchestrator can route requests to this server. For example, `Use Dynamics 365 Sales tools to manage opportunities, leads, quotes, accounts, contacts, and sales data.` |
   | Server URL | `https://agent365.svc.cloud.microsoft/mcp/environments/<environment-id>/servers/mcp_D365CX_Sales` |
...
1. Sign in with an account that has access to the target Sales environment.
1. Add the tool to the agent.
1. Publish the agent.

Optionally, if you want to use your agent in Sales, go to the **Channels** tab and connect the agent to Dynamics 365 Sales.
...
1. Add the Sales MCP server URL.
    ...
    ```json
    {
      "servers": {
        "d365-sales": {
          "type": "http",
          "url": "https://agent365.svc.cloud.microsoft/mcp/environments/<environment-id>/servers/mcp_D365CX_Sales"
        }
      },
      "inputs": []
    }
