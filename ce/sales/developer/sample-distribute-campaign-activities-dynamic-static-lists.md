---
title: Distribute campaign activities to dynamic and static lists (Sample)
...
1. The `Campaign` method creates a campaign. 
1. The `AddItemCampaignRequest` method adds the dynamic list to the campaign.
1. The `CampaignActivity` method creates a campaign activity to distribute fax to the list members.
1. The `AddItemCampaignActivityRequest` method adds the dynamic list to the campaign activity.
1. The `CopyDynamicListToStaticRequest` method copies the dynamic list to a static list.
1. The `DistributeCampaignActivityRequest` method distributes the campaign activity to the marketing lists.
