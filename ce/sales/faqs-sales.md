...
This is by design. Disable the auto-save option in Dynamics 365 to show synchronized appointments from Outlook as saved in Dynamics 365.
...
Dynamics 365 Sales detects duplicates with an AI model that compares leads using fuzzy logic as well as the matchcode that's generated for each lead.
...
However, this will interfere with your forecast and rollup values, and isn't an ideal practice. For example, if you manually set the value of an opportunity to **Won** without closing the opportunity, the opportunity might not even show up in the forecast. By default, forecasts are configured to show an opportunity as **Won** only when the actual revenue and actual close date are available.
