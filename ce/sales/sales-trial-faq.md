...
Though you can sign up for the trial from any region, the Sales Insights functionalities are restricted to specific regions. For information on supported regions, see [In which region is Sales Insights available?](faqs-sales-insights#general). If you are not in a supported region, you can only access the Sales core functionalities.   
