...
## Troubleshoot : Dynamics deep links propagated in e-mails won’t open on mobile phones and tablets (especially on Android devices)
Dynamics deep links (handcrafted or generated by workflow tools) you receive by e-mail may not open properly on mobile devices because of security restrictions most mail clients put on the displayed links.

An identified workaround consists in wrapping the url in a handmade html file.
For example :
   `<html>
...
Send this html file and, when clicked, the link will open properly in Microsoft 365 Dynamics on mobiles.

## Troubleshoot : Dynamics Deep link may not open on mobile phones in the case of an Email based Sign in process.
Deep links are always invoked in the context of the currently opened application module. While the mobile application doesn't need to be running at the time the deep link is invoked, the user must have been in an app module in the same organization when he exited the mobile application.
The known work around for this issue is to have an application of the same organization already opened on the device.

## Native Android or iOS links
Please note that "Universal Links" on iOS and "Android App Links" on Android are not supported in Dynamics mobile.
