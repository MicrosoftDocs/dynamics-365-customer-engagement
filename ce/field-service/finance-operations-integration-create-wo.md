...
1. Add the products and services that are needed for the work order and that are relevant to the work order's **Owning Company**. For account-based work orders, populate Owning Company from the selected Service Account or Billing Account. For project-centered work orders, populate Owning Company from the selected Project. For work orders without Service Account, use Owning Company to provide the company context required by the integration.
...
Agreements also include a required **Owning Company** field. Field Service can automatically populate the Owning Company on an agreement based on the selected **Service Account** or **Project**. If you have access to only one company record, Field Service can populate the Owning Company from that company.
