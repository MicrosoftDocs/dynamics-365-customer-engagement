...
For many organizations, using RSO for automated scheduling is a requirement because scheduling a high volume of jobs manually would take too long. For other organizations, they adopt RSO to aid their schedulers in job matching and to free up their time to focus on other tasks. Here are other reasons organizations choose to adopt RSO.
...
RSO is very configurable based on your business needs and you have the option of running RSO differently based on jobs, resources and different factors. Here are common scenarios where RSO is used.
...
- **Schedules, bookings** - schedules and bookings are used interchangibly and refer to the appointment time slot when a resource is assigned to a job.
...
- **Scope** - this is the jobs and resources RSO will consider. As an example, you may not have RSO schedule every job in the system to every resource. You will most likley partition the jobs and resources into a scope.  
...
In the image below is an optimized schedule board. In the left pane you'll see a map view of the optimized bookings. 
