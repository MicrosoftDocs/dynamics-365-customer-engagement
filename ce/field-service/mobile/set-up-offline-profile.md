...
- Set up item association by creating relationships between tables. You don't need to create a filter for every table because associated tables follow the filters defined the related tables.
...
1. Review the **Data for offline use**. For each table:
   ...
   - Select the rows or filters, relationships, files, and images to make available offline. For more information, see [Apply filters](/power-apps/mobile/setup-mobile-offline#add-a-table-to-an-offline-profile-and-apply-filters).
     ...
     **Advanced option**: To improve offline performance for large datasets, you can review and manually edit Fetch XML used by offline filters. Select **Edit filter** to adjust filters through the [FetchXML editor for offline profiles](/power-apps/mobile/fetchxml-editor).

   - Select the sync frequency.

     The default offline profile is updated periodically as part of Field Service updates. If you edit a table's offline sync filter, the sync filter isn't updated. Table sync filters that you didn't edit are updated, but the updates are unpublished. Administrators can review the updates and choose whether to apply them or continue using with the existing sync filters. This process only applies to sync filters. Relationships receive updates while keeping your specific changes.
...
If you have user roles that need different sync settings or tables available offline, you can [create more offline profiles](/power-apps/mobile/setup-mobile-offline#set-up-a-mobile-offline-profile). For example, a Field Service manager might need to view a broader scope of work orders than the ones that are assigned to a field technician. If you create an offline profile, add it to the Field Service mobile app in the app designer.
...
To manage offline profiles changes consistently, your organization might require that you update profiles in one environment and then move them into other environments. Publishing changes to an offline profile creates an active solution layer. Review your solution management and layering strategy when updating profiles across environments.
...
1. In left navigation pane, select **Objects**.
...
1. Go to the solution's **Overview** page, or the list of solutions.
...
1. Confirm that the mobile offline profile is available and works as expected in the target environment.
