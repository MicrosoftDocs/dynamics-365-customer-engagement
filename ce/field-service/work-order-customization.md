...
This article lists common considerations when customizing the work order form in Dynamics 365 Field Service. In addition, this article lists code components or PCF controls that are available in the form designer when editing a work order form. Makers can use these components on their forms and views to customize the experience for their users. Learn more in [Code controls in the Power Apps developer documentation](/power-apps/developer/component-framework/custom-controls-overview).
...
Configure work order subentities to pop out as dialog boxes and edit them without leaving the work order form. This configuration helps dispatchers keep the work order context. Administrators can enable the **Work Order Subgrid Records Open as Popups** setting in [**Field Service Settings** > **Other**](configure-default-settings.md#other-settings).

## Simplified work order commands

The work order command bar uses the simplified view by default and shows only the most relevant commands. To show all commands, administrators can disable the **Show Simplified Work Order Commands** setting in [**Field Service Settings** > **Work Order / Booking**](configure-default-settings.md#work-order--booking-settings).
...
1. In the solution's `customizations.xml` file, go to the `formxml` section.
...
1. Add the control with the following code to the row and cell where you want to add it.
...
1. Sign in to [Power apps](https://make.powerapps.com) and select the environment where you want to customize the work order form.

1. Open your solution and select **Tables** > **Work Order** > **Forms**.

1. Select the form you want to customize and select **Edit**.

1. Select **Component** and search for the component you want to add. Drag and drop the component to the form.

1. Save and publish the form.

### Work order Copilot recap

A [control that summarizes work orders](enable-work-order-recap.md) and associated bookings, including timeline notes, with the help of Copilot in Field Service. Copilot currently can't summarize custom fields. It only summarizes default fields.

This control works on the default and custom work order forms, but not on non-work order entities.

### Work order status

A control to select the work order status and substatus. You can [add custom system substatuses](work-order-status-booking-status.md). Field Service doesn't support custom system status. If you add one, it doesn't have a status icon. You can't change colors for the system status.
...
A control to [set the priority of a work order](set-priorities.md). You can add custom priorities and color.
...
Ensure [cost and pricing features are enabled](configure-default-settings.md#work-order--booking-settings) to see data in this control. You can set a different not-to-exceed (NTE) proximity value for this control than the one set for the work order.
