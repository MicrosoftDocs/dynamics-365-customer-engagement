...
When an author chooses a template to create an article, certain fields in the template are prepopulated with values, so the author can focus on the main content of the article.

## Create a template

Create knowledge article templates by defining the necessary fields such as Language, Title, Keywords, and Subject.
...
1. Enter a name for the template and owner.

1. Under **Article Data**, enter the values for the required fields that you want to prepopulate when the template is selected.

1. Under **Content**, establish a structure to use for any knowledge article created from this template.
