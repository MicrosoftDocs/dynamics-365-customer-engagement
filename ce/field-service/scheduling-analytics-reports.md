...
| Utilization rate |The percentage of time a resource spends on work and travel in the given period from slicer. (Work duration +travel duration) per available time on the calendar rule. Note: if the user does not set up the available time on the calendar then the default available time is 24 hours |
...
| Resource utilization breakdown  |	The visual displays the percentage of time on work, travel, break and idle time over a period  It can break via booking status slicer such as in progress job or completed job. Each bar in the chart shows the relative percentage of duration types in stacked columns, where the total (cumulative) of stacked columns always equals 100%. It does not show overtime values of any duration types. |
