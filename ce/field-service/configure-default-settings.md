...
| Cancel current slots when moving | Select whether, when moving slots to another time, to leave the old slots and change their status to **Cancel**. Please note that this behavior is only supported on the bookings that are moved through **Move bookings** functionality on the schedule board.    |
