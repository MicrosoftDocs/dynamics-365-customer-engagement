...
The integration uses a reliable asynchronous transaction framework to ensure that Field Service updates are accurate and reliable.
...
- Each transaction on a work order is committed in Field Service before creating an update in finance and operations applications.
  ...
  - If the transaction still fails to synchronize, the error and transaction details are preserved in the finance and operations transaction record. Users can troubleshoot the issue and resync the transaction ensuring no loss of financial and inventory transactional data.
     ...
     With correct system configuration, transaction failures are exceptions. Resolving this type of issue is critical to ensuring that transactional consistency is maintained between the two systems.
...
The integration supports storage dimensions when correctly configured. Storage dimensions define the levels of detail used for an item's storage in inventory. Some items are tracked at the site, warehouse, and location levels, while other items are tracked only at the site or site and warehouse levels.

Depending on the inventory product you select when creating a work order product, the defined storage dimensions determine whether you need to specify a location in the work order product.

- When you add a work order product where the chosen product has **Inventory** for the **Field Service Product Type** in Field Service, you can select a warehouse and a location from the lookup fields on the transaction form. The warehouse and location fields are filtered to show the values from finance and operations applications, based on the legal entity to which the work order's service account belongs. The configuration of the finance and operations product and its storage dimensions determine if the work order product location field is required when marking a product as used.

- When you modify or delete the corresponding work order product record in Field Service, the integration updates the relevant journal line.

The warehouse and location fields in Field Service relate to the warehouse and location concepts in finance and operations applications. These concepts track the physical locations of the inventory items.

> [!NOTE]
> The integration doesn't require you to populate the **Site** value. Instead, it populates site based on the selected warehouse, which has a hierarchical relationship with site. Configure default order settings to minimize errors when you create a work order product with an inventory product.

## Inventory

When you [enable the finance and operations integration from the Field Service Settings](finance-operations-integration-setup.md#enable-the-integration-from-field-service), Dynamics 365 Supply Chain Management becomes the system of record for inventory. This integration suppresses Field Service's default inventory functionality.
...
These new inventory items use [virtual tables](/dynamics365/fin-ops-core/dev-itpro/power-platform/virtual-entities-overview) to expose inventory data directly from Supply Chain Management inside Field Service. Users can access the true inventory levels from the system of record.

When you enable the integration, it disables the inventory validation function against Field Service's default inventory. The Field Service setting for **Use of Products Out of Stock** is hidden to ensure users don't receive irrelevant inventory validations or blocking behavior.
...
When you enable the integration, the inventory views include a version for organizations that don't use product variants. This inventory view removes several columns and simplifies inventory visibility.

For organizations that use product variants, relevant views show all the details of the inventory levels, including columns for size, color, style, and configuration. If necessary, organizations can modify the default views as they can with any table.

> [!NOTE]
> Unlike normal tables, individual rows in these inventory tables (*mserp_inventorysiteonhandv2entity* and *mserp_inventwarehouseonhandv2entity*) don't support opening in a form. The view control used in these views suppresses the ability to open these records in a form.
...
- Autopopulate the worker value on the work order product or service based on the bookable resource's worker in the transaction's associated booking. You can manually populate or override this value.
...
- Require the worker on noninventory related work order products or work order services. The requirement is based on whether the company's associated project management and accounting parameters are configured to require a worker value in hours journal lines or expense journal lines.
  ...
  If you configure the worker requirement in finance and operations apps, Field Service only requires a value when you set the work order product or service to used. This requirement doesn't block creating estimated records without knowing the eventual worker. The transaction fails to sync unless the worker is recorded.

The worker field isn't filtered based on the eligibility of the worker to perform work on the related project. Eligibility isn't considered as a scheduling parameter when determining the bookable resource to schedule on a work order. As a best practice, consider using security roles and business units or introducing resource characteristics to ensure workers are only scheduled for relevant work orders. Consider these options if it's a critical consideration for your organization's implementation of finance and operations applications.
...
The integration adds several fields to the work order **Product** table. You can see these fields when the integration is installed and enabled.

- **Journal Description**

  - Visible only for expense transactions (noninventory products). Hidden for inventory products (which result in item journals).
  - Autopopulates with the product name of the applied product, but you should override the value with a more meaningful description.
  - Limited to 60 characters. This field flows from the record in Field Service to the journal line to the invoice generated in Finance.

- **Line Property**

  - A virtual table lookup that's filtered based on the relevant company or legal entity.
...
- **Location**
  ...
  - Seen only if the inventory dimensions you configure in finance and operations applications on the product indicate location is required.
  - Only required when you set the record to used.
  - A virtual table lookup that's filtered based on the relevant company or legal entity and selected warehouse.

- **Transaction Date**

- **Worker**

  - Only required based on project management and accounting parameters you configure on the relevant company.
...
The integration adds several fields to the work order **Service** table. You can see these fields when the integration is installed and enabled.

- **Journal Description**

  - Automatically populates with the product name of the applied product, but you should enter a more meaningful description.
...
- **Line Property**
  ...
  - A virtual table lookup that's filtered based on the relevant company or legal entity.
  - The integration hides the **Duration to Bill** field because there's no equivalent concept on journal lines. Instead, line property defines whether a transaction is billable.

- **Transaction Date**

- **Worker**

  - Required only when project management and accounting parameters on the relevant company need it.
  - Automatically populates if the work order service's booking value is populated based on the worker of the bookable resource set on the booking.

### Product field additions

The integration adds a virtual table lookup field called **Project Category**. It's filtered based on the relevant company or legal entity and whether the product is an inventory (for item categories), noninventory (for expense categories), or service (for hours categories) product.

## Work order transaction handling

When events occur with a Field Service work order’s product and service records, the integration triggers corresponding transactions in their respective journal and journal line records.

### Create

- **Product**: When you create a work order product record, the integration creates either an item journal and journal line or an expense journal and journal line. The product's **Field Service Product Type** determines whether the transaction triggers an item or expense transaction.

  - Inventory products trigger items.
  - Noninventory products trigger expenses.

- **Service**: When you create a work order service record, the integration creates an hours transaction.

### Update

- When you update a work order product or service and the related journal and journal line are unposted, the integration updates the transaction appropriately.

- When you update a work order product or service and the related journal and journal line are posted, the integration:
...
- When you delete a work order product or service and the related journal and journal line are unposted, the integration deletes the journal and journal line.

- When you delete a work order product or service and the related journal and journal line are posted, the integration creates a reverse transaction against the original journal and journal line.
...
- When a work order is updated to **Canceled** or **Posted**, the integration deletes all unposted journals and journal lines.

- When a work order is updated from **Canceled** or **Posted** to any other system status, the integration recreates lines for estimated lines that it previously removed.

## Limitations

This integration supports the use of [Microsoft-managed](/dynamics365/fin-ops-core/dev-itpro/deployment/cloud-deployment-overview#customer-lifecycle-subscriptions-and-environment-types) environments. It doesn't support customer-managed environments.
...
[Project Operations resource/non-stocked integration](/dynamics365/project-operations/environment/resource-dual-write-overview) doesn't allow the Field Service integration to work with the same legal entities that are enabled for the resource/non-stocked integrated scenario. However, it can work in the same environments for other legal entities.
...
- The **Require Activity On Journals** settings in a Project set to "Yes" isn't supported. If you configure activities for Hours, Expenses, or Items as required, the transactions from Field Service don't succeed.
