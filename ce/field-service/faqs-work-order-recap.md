...
Use work order summary to quickly understand the state of a work order at any stage of its lifecycle. The feature provides a context-specific summary that helps you decide what actions to take or what details to investigate further. It's not a substitute for a thorough review of the work order and its related information.
...
The feature was tested in English. For detailed information on the supported languages, [Explore Copilot features by geography and languages](https://releaseplans.microsoft.com/availability-reports/?report=copilotfeaturereport).

The accuracy and performance of the summary were checked based on the data from the work order. The summary quality depends on the quality of the data in the system. If the data is incomplete or incorrect, the summary might reflect that.

No cases were found where the summary made up information that wasn't in the data. However, some cases were found where the summary didn't summarize the data correctly.

## What are the limitations of the feature and how can I minimize their impact?

You can help by giving feedback using the like or dislike buttons on the summary. Microsoft uses this data to evaluate and improve the quality of the content.
...
Administrators can enable or disable the feature for your environment in the Field Service settings by toggling the **Copilot for work order** option.

To customize the summary, an administrator can enable **configure summary** to choose the fields that Copilot uses to generate the summary. You can't customize other aspects of the prompt. However, the feature respects your personalization settings for date and time format and time zone, as well as your environment-specific table and field labels. Administrators should test the chosen configuration with data from a record before saving the change.
...
The form component is called **Copilot recap**. By default, it's available on the new work order experience form. An administrator can [add the control to other work order forms](/power-apps/maker/model-driven-apps/additional-controls-for-dynamics-365-for-phones-and-tablets#using-controls-in-the-form-designer).
