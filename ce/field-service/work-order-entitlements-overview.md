...
description: Entitlements for work orders in Dynamics 365 Field Service help apply price lists and discounts by account, asset, products, services, and incident type.
ms.date: 09/16/2026
ms.topic: how-to
applies_to: 
  - "Dynamics 365 (online)"
...
You can apply multiple entitlements to a single work order product or service. Apply entitlements based on different work order and work order product and service attributes. For example, an entitlement can apply based on a work order billing account or a customer asset category. To learn how to create an entitlement for both of these scenarios, see [Create entitlements for work orders](work-order-entitlements.md).

In such cases, all applicable entitlements show as options in the **Entitlement** lookup field for the work order. However, the system selects one entitlement by default. The system looks at the **Entitlement Prioritization** values of all applicable entitlements and selects the one with the highest priority (1 is the highest priority). Any priority value is greater than a blank priority value.
