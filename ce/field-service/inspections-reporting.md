...
First step would be to configure how often inspection response should be parsed and organized in Dataverse.
...
- **Custom**: Define your own frequency in number of days interval when parsing should be done. See the following screenshot for an example.
...
Now that analytics is configured, questions and responses are ready to be saved into the three tables in Dataverse:
...
These are the 3 Flows involved:
...
The status of the Flows can be checked as shown in the snapshot. 
> [!div class="mx-imgBorder"]
