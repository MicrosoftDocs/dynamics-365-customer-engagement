...
| Bot Resolution Rate  | The percentage of conversations that were resolved by a bot out of all conversations handed by a bot. | All conversations are considered. | FactSession[BOTResolutionRate] |
| Bot Resolution time (mins) | The length of time, in minutes, a customer interacted with a bot before the issue was resolved. | All conversations are considered. | FactSession[AvgResolutionTime] |
...
| Bot Escalation time (mins)  | Average length of time that a bot took to complete the conversation with a customer before escalating to a human agent. | All conversations are considered. | FactSession[BotEscalationTime] |
