...
2. On the **Active Conversation** page, locate the **Mobile Phone** field, and then select the call icon to call the customer. You can also select the **Launch dialer** phone icon on the menu bar to quickly call a customer.
     ...
     - When a country is chosen from the dropdown, the application populates the appropriate country code in the phone number field. If the country code is changed while making an outbound call, the new country becomes the default.
