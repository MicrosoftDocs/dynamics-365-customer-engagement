...
### Metrics by bot 
 ...
 | Title | Description |
 | --------------- | --------------- |
 | Total sessions| The total number of Copilot Studio bot sessions within the selected time period. Since each conversation can include one or more sessions, the session count may differ from the conversation count. If a conversation escalates to a representative, the Copilot Studio session ends at that point. |
