...
- Make sure that the **Allow conversation transcripts and their associated metadata to be saved in Dataverse** is selected in Power Platform Admin Center. This setting is required for transcripts to be saved in Dataverse. If this setting isn't enabled, the bot dashboard that uses metadata from conversation transcripts might not work as expected.
