...
|`StartTimeOfNextOperatingHour`           | DateTime    | The start time (UTC) of business hours for the queue if it is outside operating hours​. No value is returned during operating hours.|
|`EndTimeOfNextOperatingHour`          | DateTime    | The time (UTC) when business hours end for the queue. No value is returned during operating hours.|
| `nexttransitiontime`        | DateTime      | The time (UTC) when the queue will be available next.|
