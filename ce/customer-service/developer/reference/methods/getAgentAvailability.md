...
> - This method can be called only 100 times per minute per organization.
...
|`StartTimeOfNextOperatingHour`           | DateTime    | The start time (UTC) of business hours for the queue if it is outside operating hours​. During operating hours, a value of 01-01-0001 is returned.|
|`EndTimeOfNextOperatingHour`          | DateTime    | The time (UTC) when business hours end for the queue if it is outside operating hours. During operating hours, a value of 01-01-0001 is returned.|
| `nexttransitiontime`        | DateTime      | The time (UTC) when the queue is operational next if it is outside operating hours. The time when the queue is not operational is displayed during business hours.|
