...
The `isDisplayable` property, when set to `true`, will display the context variables in the third tab on the Conversation Summary Control for a given conversation. The `Value` property of the JSON object includes the user response or any other information in string, Boolean, or number format.
