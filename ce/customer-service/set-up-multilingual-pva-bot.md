...
1. In the site map, select **BeginDialog**, select the **Add** (+) node, and then select **Send a response**.
   ...
   1. Select the **Add** (+) node, select **Create a condition**, and then select **Branch Switch (multiple options)**. Enter the following details:
        ...
        > [!NOTE]
        > You must set the va_CustomerLocale context variable with the locale codes for the languages that you want to support. More information: [Supported locations and locale codes](voice-channel-regional-availability.md#supported-languages-and-locale-codes)
