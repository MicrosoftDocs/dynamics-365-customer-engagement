...
1. Install the Dynamics 365 Customer Service mobile app.
  ...
  To install the Dynamics 365 Customer Service mobile app, follow the steps in [Install Dynamics 365 for phones and tablets](../../mobile-app/install-dynamics-365-for-phones-and-tablets.md)
...
## Work with case management record types, dashboards, and tools
 ...
 In the Customer Service Hub app, you can find everything you need to help you easily manage customer service for your organization:  
...
When you select a record type from the menu, such as Accounts, Contacts, or Cases, a list appears with corresponding customer records (also known as an entity grid).
...
> [!NOTE]
> The option Open Record Set won't be available on devices with comparatively smaller screen sizes.
...
When you select a process stage on the process bar, the system shows the steps to take to complete that stage. A flag on a stage indicates the stage you're currently on. When you select an active stage, the **Next Stage** button moves to the next stage in the process.  

The stages in the business process flow are displayed in floating mode, or docked mode. Stages can  be aligned in a vertical layout, instead of horizontal, which makes it easier to review the required steps to move forward. You can also choose to include an optional step in the business process flow and define criteria to trigger it.
...
Each case form is composed of one or more tabs. By default, the **Summary** tab shows information about the customer, interactions with the customer, and other related records. You can select different tabs to enter or review other miscellaneous details of a record.  
...
The contact card displays a complete view of your customer's touch points. This card is available in the **General Information** section of the **Summary** tab.  
  
![Contact card.](../media/contact-card.png "Contact card")

You can add a picture to the account or contact record in Dataverse, and it appears on this card. You can make calls to customers from the Contact card using Skype for Business – IP telephony.
...
Use queues to organize, prioritize, and monitor the progress of your work. In Dynamics 365 Customer Service, queues are containers that are used to store anything that needs to be completed or requires an action. For example, you could use queues for completing a task or closing a case. Learn more about creating and managing queues in [Create and manage queues](../administer/set-up-queues-manage-activities-cases.md).
...
In the **Related** section (also known as Reference panel), use the tabs to review related records for the current record. This section is enabled by default for account, contact, case, and knowledge article records. Your customizer can add this section for all entities (including custom entities) that are enabled for interactive experience. The reference panel is also available in model-driven apps. Learn more in [Reference Panel in model-driven apps ](/powerapps/user/navigation#reference-panel).
...
> [!NOTE]
> You can configure rules that help you view similar cases based on automatic suggestions. Learn more in [Use advanced similarity rules to view similar case suggestions](../administer/suggest-similar-cases-for-a-case.md).
...
- Select **Due** to review the list of activities that are due in coming time or are already overdue.
...
Use queues to organize, prioritize, and monitor the progress of your work. In Dynamics 365 Customer Service, queues are containers that are used to store anything that needs to be completed or requires an action. For example, you might use a queue for completing a task or closing a case. Learn more about how to create and manage queues in [Create and manage queues](../administer/set-up-queues-manage-activities-cases.md).
