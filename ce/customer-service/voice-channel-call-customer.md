...
2. On the **Active Conversation** page, locate the **Mobile Phone** field, and then select the call icon to call the customer. You can also select the **Launch dialer** phone icon on the menu bar to quickly call a customer.
     ...
     - On the dial pad, you can copy or type in a number or select a number from the recently called numbers. The dropdown displays the list of supported countries and regions that you can call. You can also view and call the most recently called numbers. By default, the last 20 calls that you’ve received or made are displayed.
     ...
     - You can only call the regions that are set in the outbound profile by your administrator. If you have multiple outbound profiles set, the profile that has the region assigned is displayed to call the customer.
