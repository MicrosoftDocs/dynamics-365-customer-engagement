...
- The **Applicable When** and **Success Conditions** don't work as expected if they're configured for related entities.
    ...
    - The **Applicable When** conditions, if defined on the related entity, work only for the first time when the conditions are met, and the KPI instance is set to **In Progress**. After that, any update to the related entity won't cancel the existing KPI instance if the conditions of the same SLA item are false. The update doesn't create a new KPI instance if there's a change in the SLA item.
...
- For expired SLAs, the SLA success date isn't available even if the success criteria defined on the SLA Item are met as there's no field on the SLA KPI Instance that's populated with the date of the action.
