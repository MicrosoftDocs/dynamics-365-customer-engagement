...
|`data-enable-sso-magic-code` |  Use this data tag to enable a verification code to be presented to the user after sign in. This is similar to multifactor authentication and helps prevent phishing attacks. You can set this tag to `true` (default) or `false`. If set to `true`, the user will be presented a verification code to enter back into the chat, after they sign in. If set to `false`, then the code is automatically sent through the client silently, after the user signs in. |  Add this tag to the widget script as shown in the following example to automatically send the verification code to the user after sign in. <br /><br />`<script id="Microsoft_Omnichannel_LCWidget" src="<valid-src>" data-app-id="<valid-app-id>" data-org-id="<valid-org-id>" data-org-url="<valid-org-url>" data-enable-sso-magic-code="false"></script>`|
||||
