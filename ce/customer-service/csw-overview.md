...
<!--note from editor: I know you said not to edit this, but the style guide doesn't like "Shift + mouse click".-->
...
<!--note from editor: These edits are suggested to make it clearer who is being addressed in this section. If I read it correctly, it alternated between agent and admin, and I think it should always be obvious to the reader if they're the "you" the article is talking to.-->
When an administrator enables simplified navigation, the navigation options will have the following characteristics:<!--note from editor: The headings in this section weren't parallel, as far as I could see. Because it seemed that this was basically a list of the new navigation behaviors, I "flattened" it into bullets rather than having those un-parallel headings. I'm not able to picture how this navigation works, exactly, but I think there's some repetition in this list that maybe can be eliminated?-->
...
- Selecting Shift while clicking overrides the new, simplified navigation<!--note from editor: Edit okay? If you're describing what happens after the new navigation is configured, I assume that the new behavior has become the "default navigation."-->  and opens the record in a new session. Selecting Ctrl while clicking overrides the simplified navigation and opens the record on a new tab.

**To explore simplified navigation after it's configured**
<!--note from editor: Edit okay? I assume that this procedure is directed at the agent. If it actually is for the admin, it can be moved to the end of the next section.-->
...
<!--note from editor: Edit okay? I don't think it's overkill to split these tasks out into their own H3, if they're the only things that only the admin can do.-->
> [!IMPORTANT]
