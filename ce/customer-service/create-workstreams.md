...
The feature is enabled by default for SMS, social channels, and Microsoft Teams. In these channels, when a conversation moves from the waiting to active state, it might not get assigned to the same agent that had previously handled it. You can use the **Keep same agent for entire conversation** option when you configure the work distribution for the workstream to reassign the conversation to the agent that had worked on it earlier. This helps save the effort to reorient the agent or set the context about the customer issue again. 
