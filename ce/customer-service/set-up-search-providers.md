...
In Microsoft Cloud for Sovereignty, the external search providers feature is available only in Government Community Cloud (GCC).
  
To view search results, your agents must have access to external content, or they won't be able to view them.

Use the integrated search provider option to set up third-party knowledge sources, such as enterprise websites based on the site map protocol. For more information, see [Manage integrated search providers](add-search-provider.md#manage-integrated-search-providers).
...
>[!NOTE]
>
> The [configured value for the number of article search results](./add-knowledge-base-search-control-forms.md) won't apply if you configure even a single external search provider. For each configured search provider, the search results will display three articles. Agents can select **Show more** to view additional results.
