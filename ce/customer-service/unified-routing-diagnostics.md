...
Diagnostic work item is a record which is created to log and display the details of each event related to a specific work item that passes through various routing stages before assignment. The **Routing diagnostics** page displays a list of work items with details and the stage each work item is in. A brief description of the attributes is as follows:
