...
In cases when more than one agent matches the requirement of the work item, the system resolves the assignment in a round robin manner, based on the earliest last assignment time. For example, if three agents Lesa, Alicia, and Alan with same skills and capacity are available to be assigned work, and their last assignment time stamps are 10:30 AM, 10:35 AM, and 10:37 AM respectively, the agent whose last assignment time is earliest will be assigned with the work item. In this case, the system will assign the work item to Lesa. Meanwhile, if another work item comes, the system will assign it to Alicia and not to Lesa or Alan.
