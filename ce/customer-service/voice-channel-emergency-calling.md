...
With the new emergency calling feature, agents can call the emergency number during a crisis situation. When the agent dials the emergency number, a temporary callback number is assigned to the agent. The agent can be reached via callback in case of a dropped emergency call even when using a shared outbound number. 
...
> - The callback number is temporarily assigned to the agent for a duration of 60 minutes.
