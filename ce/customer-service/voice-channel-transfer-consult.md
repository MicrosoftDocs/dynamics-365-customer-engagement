...
3. Based on your requirement, perform one of the following steps.
    ...
    - **Transfer to an external PSTN number**: Select **Number pad**. On the number pad that appears, enter the customer's country code, phone number, and then select **Call** to make your call. When a secondary agent picks up the call, you can talk to the agent while the customer is on hold. To complete the call transfer, the you must select the transfer button next to the secondary agent name in the call window. The secondary agent is now the primary agent and you will be removed from the call.<br>
    
    - **Transfer to Teams user via PSTN**: Select **Teams**, and use the search option to search for a Microsoft Teams user. The results are filtered based on the name and the live presence in Microsoft Teams. When a secondary agent picks up the call, you can talk to the agent while the customer is on hold. To complete the call transfer, the you must select the transfer button next to the secondary agent name in the call window. The secondary agent is now the primary agent and you will be removed from the call. <br> 
        > [!div class="mx-imgBorder"]
