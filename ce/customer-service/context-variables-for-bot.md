...
|-----|-----|-------|-------|-----|
|  `type`  | Set survey consent | Use this action to set the survey consent flag to the conversation that can later be managed by a secondary bot. | <ol> <li>Set the survey consent flag by adding the action to the bot, and setting to `true` or `false`.</li> <li>Make sure your conversations ends with **Transfer to agent**. </li><li>Configure a survey bot, and add it to Dynamics 365 for Customer Service. </li><li>Configure the routing rule in Dynamics 365 for Customer Service to route the call to the survey bot. </li> </ol>|       |
| `type`  | Set customer locale and handoff to Omnichannel   | to transfer the user talking to the bot to another bot, which might speak in a different language than the current bot.   |  Set the code by selecting the input variable and using the correct locale code. For an example on how to set up this scenario, see
[Configure the routing rule in Dynamics 365 for Customer Service](routing-rules.md) |
