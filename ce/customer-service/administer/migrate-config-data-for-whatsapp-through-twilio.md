...
If you use custom session templates and notifications in your WhatsApp routing setup, perform the steps outlined in [Migrate configuration using a solution](migrate-record-routing-config-using-solutions.md).
...
1. Use the Configuration Migration tool to create the schema and migrate the data from the source organization for the record workstream.
   ...
   - **Use FetchXML to filter records**: For each entity, use the appropriate FetchXML query in the following table to get single, multiple, or all records based on your requirement. For single or multiple records, use source organization to get the correct name in uiname and GUID in value. If necessary, you can use the ADVANCED FIND menu item to construct the appropriate FetchXML query.
...
1. Make sure that the workstreams that are referred in the configuration of the WhatsApp channels are included in the migration.
...
> [!IMPORTANT]
> When you fetch decision rulesets (msdyn_decisionrulesets) containing context variables, some decision rulesets are created without the dependency mappings. To work around this behavior, perform the following steps:
