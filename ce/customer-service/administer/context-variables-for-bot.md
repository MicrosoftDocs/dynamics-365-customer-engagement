...
> [!IMPORTANT]
> - [Install the extension solutions](/power-virtual-agents/configuration-hand-off-omnichannel#install-extension-solutions) to improve access to certain variables.
...
| Messaging-only variables               | msdyn_CustomerType **<sup>1</sup>**                         | Use this context variable that contains the customer type for the *account* or *contact* record linked to the conversation. | Select the Omnichannel for Customer Service extension solution variable.                                         |
|                                        | msdyn_CustomerName **<sup>1</sup>**                        | Use this context variable that contains the customer name for the *account* or *contact* record linked to the conversation. | Select the Omnichannel for Customer Service extension solution variable.                                         |
...
| Voice-only variables **<sup>2</sup>**  | Activity.From.Name                        | Use this context variable for the customer's phone number in the Telephony activity. Assign this variable to a global variable in the **Conversation Start** topic. Learn more in [Configure context variables for Copilot Studio agent](#configure-context-variables-for-copilot-studio-agent).| Select the Omnichannel for Customer Service extension solution variable.  |
|                                        | Activity.Recipient.Name                   | Use this context variable for the organization's phone number in the Telephony activity. Assign this variable to a global variable in the **Conversation Start** topic. Learn more in [Configure context variables for Copilot Studio agent](#configure-context-variables-for-copilot-studio-agent).                       | Select the Omnichannel for Customer Service extension solution variable.                                         |
