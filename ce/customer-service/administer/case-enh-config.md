...
> - The **Enhanced case form** is available in the form selector by default as an early access feature. When you switch from the enhanced case form to any other form, the customer details pane isn't automatically refreshed or closed since it is supported only for Enhanced case form. You will need to close the pane manually. You can opt in early to enable these features in your environment, which will allow you to test these features and then adopt them across your environments. For information about how to enable these features, see [Opt in to early access updates](/power-platform/admin/opt-in-early-access-updates).
