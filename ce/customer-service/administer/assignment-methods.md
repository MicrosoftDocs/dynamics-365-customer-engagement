...
When agents are offered a work item through automatic assignment, they typically can accept or decline. Both [rejection](enable-agent-reject-notifications.md) and [time out](manage-missed-notifications.md) of the notification is considered as a decline. If an agent declines a work item through one of the methods, they are not prioritized for the declined conversation. If the agent is the only one with right set of skills for the deciined conversation or if round robin assignment strategy is being used, the agent is considered for the declined conversation thrice. If the agent declines the same work item thrice, they aren't considered for further auto assignment for the specific work item. The system tries to offer the declined work item to other agents in the queue if they're eligible.
