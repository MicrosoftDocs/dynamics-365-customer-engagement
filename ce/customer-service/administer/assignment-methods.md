...
- **Highest capacity**: Assigns a work item to a representative with the highest available capacity. This representative has the skills that are identified during the classification stage and presence that matches one of the allowed presences in the workstream. If more than one representative is available with the same capacity, the work item is assigned based on the round-robin order of the ones with the same highest capacity.

  If you want to use skill-based routing, the "exact match" and "closest match" options are available.

  - If you set **Default skill matching algorithm** in the workstream as **Exact Match**, then the system filters representative using exact skill match, workstream’s presence, and capacity requirements, and orders the filtered representatives by available capacity.
...
- **Least active**: Assigns a work item to the representative who has been least active among all those who match the required skills, presence, and capacity.

  The assignment method uses "the time since last capacity is released for a voice call" and the wrap-up settings configured in the workstream to determine the least-active representative and route the next incoming call to them. For example, consider two representatives in a queue. The first one completes a call five minutes ago while the second one has just completed their call. When a new call comes in, the system assigns it to the first representative who has finished their activity first.

  Routing to the least-active representative assignment strategy helps in a balanced distribution of work items across them, and results in higher representative efficiency and improved customer satisfaction.
...
- **Create new**: Lets you create and use your own rulesets and rules to configure priority, severity, and capacity for choosing the queues to which work items need to be routed. You can create the following rulesets:
  ...
  - **Assignment rulesets**: Represent a set of conditions that are used to select, and use an order by option to sort, the matching representatives.
...
- Addition of an representative to the queue.
...
The assignment ruleset is an ordered list of assignment rules. Each assignment rule represents a set of conditions that is used to determine the selection and an order-by field to sort the matching representatives. At runtime, the assignment rule with the top order is evaluated first. The representatives are matched as per the conditions specified in the rule. If more than one matching representative exists, they're sorted by the order by field, and the top match is assigned the work. If no representatives are matched, then the next assignment rule in the ruleset is evaluated. This method can be thought of as a gradual relaxation of constraints in the assignment, such that first, the strictest criteria are applied, and then the conditions are reduced so that the best representative is found. If no matches are found, then the work item remains in the queue.
...
- **Condition**: The expressions that are evaluated to match the users with the attributes of incoming work. The conditions have three parts:
  - **User attribute**: Properties of the users that can be used for comparing the user with the incoming work. The user attributes can be one of the following:
    ...
    - **Bot attributes**: Can be used only when you have configured AI agents as users and want to do some comparisons on them.
...
- **Order by**: If multiple representatives match the conditions in a rule, you can use the "Order by" clause to find the best-suited one. You can specify the following order by clauses:
  ...
  - **Ordering Attributes**:
    ...
    - **Least active**: Is available for voice queues only. The work item is routed to the least active representative among all who match skills, presence, and capacity. Learn more in the [Types of assignment methods](#types-of-assignment-methods) section.
    - Round robin
...
If the the same work is declined item three times or reaches the configured limit, the representative is no longer considered for auto assignment of that particular work item. The system then attempts to assign the declined work item to other eligible representatives in the queue. They can still manually pick the work item.
