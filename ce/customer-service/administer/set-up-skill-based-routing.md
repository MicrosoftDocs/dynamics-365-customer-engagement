...
When a workstream is configured with 'exact match' skill matching and an out-of-the-box assignment method is set, if multiple agents possess the exact skills required for a work item, the system will rank them based on the assignment method. The work item is then assigned to the representative based on this ordering.
- **Custom assignment**: In the custom assignment rule conditions, create a rule with **User skills**, select **Exact match** and then select **All skills** or a specific skill type. Use **All skills** if you want to find an agent matching all the skills that are attached to the workitem. This lets the assignment logic find representatives who either match all required skills or match all skills within a selected skill type.
The exact match algorithm configured in workstream is not applicable if custom assignment method is applied to a queue. You'll need to write rules to achieve exact match for the custom assignment method. 
...
In this scenario where closest match is chosen as the skill matching algorithm at workstream level, and an out of the box assignment method is chosen; if more than one agent with same skill score is matched to the work item; the system will order these matching agents based on the chosen assignment method and assign to an agent based on the ordering. 
