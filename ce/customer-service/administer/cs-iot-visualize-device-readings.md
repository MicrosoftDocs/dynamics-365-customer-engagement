...
To help you understand the current state of a device, its repair history, and the effect of performed work orders, the Connected Customer Service add-in displays near real-time device readings and historical sensor measurements in a chart alongside work order details.
...
- Currently available in United States English only. This feature is in **preview** and it isn't available in all languages and geographies.
...
| 4000103	 |	Value for the From or To date argument passed to the custom action msdyn_IoTHubQueryDeviceReadings isn't a valid DateTime |		Verify that valid values for all the required arguments are passed to the custom action msdyn_IoTHubQueryDeviceReadings | 
