...
1. Select the **Transfer to a different queue** action, and then select a queue in the dropdown list.
...
- Messaging and record channels: 1 minute to 2 days

 The wait time that you specify must be at least one minute. For any other value, the time is rounded off to the nearest 30 seconds.
...
The overflow action applies to all open work items, regardless of how they were placed in the queue. That includes work items that were transferred by an agent or supervisor or that were routed from another overflowing queue.
