...
3. Extract the zip file, open the data.xml file present in the extracted folder, and perform the following operations:
   ...
   - In the source and target organizations, run the following OData API call and note the `msdyn_decisioncontractid`:
     ...
     If there is no output when you run the call, skip to step 4. If there is an output, then in the data.xml file, replace all occurrences of the msdyn_decisioncontractid GUID in the source organization with the msdyn_decisioncontractid GUID of the target organization.
