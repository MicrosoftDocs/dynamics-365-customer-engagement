...
| Prompt and transfer to external number| The system presents an automated message to the customer that the AI agent is unable to process the request and then transfers the call to a specified external number. |You can set it up to trigger a fallback message whenever the Copilot agent becomes unresponsive at any point during the customer’s call.| Customer |
| Prompt and escalate | The system presents an automated message to the customer that the AI agent is unable to process the request and then transfers the call to a customer service representative. |You can set it up to trigger a fallback message whenever the Copilot agent becomes unresponsive at any point during the customer’s call.| Customer |
| Prompt and hang up | The system presents an automated message to the customer that the AI agent is unable to process the request and then ends the call. |You can set it up to trigger a fallback message whenever the Copilot agent becomes unresponsive at any point during the customer’s call.| Customer |
