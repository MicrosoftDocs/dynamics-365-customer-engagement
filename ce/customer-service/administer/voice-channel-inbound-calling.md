...
- You acquired a phone number. Learn more in [Manage phone numbers](voice-channel-manage-phone-numbers.md).
...
1. On the left pane, select **Workstreams**, and then on the **Create a workstream** dialog, enter the following details for the workstream:
...
1. On the **Phone number** page, select a number from the list, and select **Next**.
    ...
    > [!NOTE]
    >
    > - Only those numbers are displayed that have inbound calls enabled and aren't already associated with any other workstream. Use the steps in [Acquire a phone number](voice-channel-manage-phone-numbers.md#acquire-new-phone-numbers) if you want to configure a new number.    
...
You are presented with following transfer and consult options:
...
### [Enhanced voice](#tab/enhancedvoice)
  ...
  1. Set the toggles for **External phone number** and **External Microsoft Teams users** in **Consult** and **Transfer**. These settings allow representatives to consult with other representatives or Teams subject matter experts during an ongoing call, and also enables them to transfer calls.
  1. Select the **Use bridged transfers** checkbox. The following actions happen when a representative transfers a call to an external phone number or a Microsoft Teams user:
  ...
  1. If the **Use bridged transfers** checkbox isn't selected, then the following actions happen when a representative transfers a call to an external phone number or a Microsoft Teams user:
     - Call ends for the primary representative as soon as the secondary representative or Teams user accepts or rejects a call.
...
> [!NOTE]
> If you're a Direct Routing customer and the Use bridged transfers checkbox isn't selected, configure your Session Border Controller to accept a "REFER-TO: sip.pstnhub.microsoft.com". The controller must convert the request to an INVITE and send it back to sip.pstnhub.microsoft.com to complete the transfer. Learn more in [Call Transfer](/azure/communication-services/concepts/telephony/direct-routing-sip-specification#call-transfer).
...
Go to the workstream you configured the voice channel for, and then do the following steps:
...
1. In **Route to queues**, select the queue that you created to route voice calls when the specified conditions are met.
1. (Optional) To distribute work items across multiple queues based on percentages, configure percentage-based allocation. Learn more in [Percentage-based allocation of work to queues](configure-route-to-queue-rules.md#percentage-based-allocation-of-work-to-queues)
