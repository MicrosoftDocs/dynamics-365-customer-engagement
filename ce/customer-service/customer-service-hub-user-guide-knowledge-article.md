...
[!Note] You can access your browser's context menu by pressing Ctrl + right-click. This is useful if you need to use your browser's built-in spell checker. Otherwise, you can use the right-click to provide contextual formatting for any element you might be using.
...
|![Paragraph Format](../customer-service/media/paragraph-format.png "Paragraph Format") | Apply predefined block-level combinations of formatting options. A paragraph format can only be applied to a block-level element, like a `paragraph` or a `div` element. **Note:**  This option is available in the expanded mode only.
