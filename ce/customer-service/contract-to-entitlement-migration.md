...
>A. Allocation type to ‘Coverage Dates’ on contract template is used to allow any number of cases to be created between start date and end date of contracts. To switch from the Contract template and map to the Entitlement template:
...
>B. Fields which are required for business use case, but not available out of box on entitlement or for other required fields not available on entitlements.
...
## Runtime impact
1.	Steps to migrate from contract to entitlementMove the contract and contract lines to entitlements.
2.	Run update script to update entitlement on case instead of contracts.

>[!Note] 
> Contract lines with Armhand 100 can be associated to the case with product ARMhand 150. With entitlement, product on entitlement should be same as product on case for entitlement to be assigned to case.
