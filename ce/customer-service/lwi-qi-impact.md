...
- For a particular queue item, when you change the queue from a (1) regular queue to an automatically routed queue or (2) from one automatically routed queue to another via the [AddToQueue Action](https://docs.microsoft.com/en-us/dynamics365/customer-engagement/web-api/addtoqueue?view=dynamics-ce-odata-9) on the entity record or by clicking the **Route To** button (via the **RouteTo Action**) on the queue item, the associated queue for the live work item is also updated to the same destination queue.
- For the queue items that have associated live work items, when you update the **Worked By** field by clicking the **Route To** button on the Queue Item grid (via the **RouteTo Action** on the queue item), the live work item is also assigned to the same agent.
- When a queue item with an associated live work item, you cannot use remove it from the queue by clicking the **Pick** button (**PickFromQueue Action**) or **Release** button (**ReleaseToQueue Action**) on the queue item.
