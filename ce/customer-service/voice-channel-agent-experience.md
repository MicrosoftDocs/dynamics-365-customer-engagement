...
- **Transfer**: View how you can transfer this call. The following options are available:
    - Transfer the call to an agent or queue. During a transfer to the queue, the customer is automatically put on hold.
...
> [!NOTE]
>  -  When a call is transferred to an agent, the app displays the primary agent's number on the caller ID.
>  - The transcription and recording of transferred calls will continue unless this option isn't enabled or is disabled by the transferring agent.
