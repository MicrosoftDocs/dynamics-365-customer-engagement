...
| 3 | Displays the confidence score based on which the similar case was selected by the AI model. The minimum confidence score is 65%. |
| 4 |  Blue bar indicates that the user hasn't interacted with the card yet. |
| 5 | When selected, links or removes the link to the similar case to the current active case. |
