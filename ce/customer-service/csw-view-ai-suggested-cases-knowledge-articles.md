...
| 6 | When user selects yes, the label is highlighted and the card stays on the pane. If user selects no, the card is replaced with a new suggestion, if it exists. This feedback from agents is used by the AI model to retrain and improve the suggestions. |
