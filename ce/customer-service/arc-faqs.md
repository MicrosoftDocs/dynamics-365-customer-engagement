...
1. Perform the following steps to check where the duplicate queue items are from:
    1. Rule item flow definition to verify if the child flow adds the email to queue. Duplicate queue items can be created because of various reasons, such as a straightforward flow step calling **AddToQueue** bound action, queue item creation, another custom flow which the ARC flow calls from within, or a post operation plugin of an action performed within ARC flow, such as case creation.
    1. Email background processes to verify if there are any custom workflows. Check this during live debugging, during which successful background processes get cleaned up. If you don't see any background processes for an email received, it doesn’t necessarily mean that there are no custom runs.
