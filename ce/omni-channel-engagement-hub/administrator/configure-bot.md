...
A bot is a program that initiates a conversation with a customer and provides automated responses. It can also help in resolving customer queries by using case deflection. A bot can also collect basic information from a customer and then provided it to a customer service agent to work further on the issue raised by the customer.
...
9.	In the **Manage User Roles** window, select **Omni-channel agent**, and then select **OK**.
    ...
    > [!NOTE]
    > You must ensure that the bot user has the highest capacity among all users. The capacity of a bot user doesn’t reduce when a query is handled by it.
...
You can add a bot user to specific queues or create a queue with the bot user only. If you create a queue with the bot user only, ensure that the routing rules are set in a way that customer queries are sent to this queue first. This ensures that the bot acts as a first line of defense for all queries.
...
3.	Follow the instructions in Step 3 to create escalation rules. Let's say you create a context variable named **BotHandoffTopic** in the **BotWorkStream** work stream. 

4.	Create three routing rules in the **BotWorkStream** work stream in the following order:
...
When a chat is initiated by a customer, the query is routed to the bot through the **BotRule** routing rule. If the bot escalates the query, it is sent to the appropriate agent as per the configured routing rules.
