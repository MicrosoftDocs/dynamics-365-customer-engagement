...
## Send a bot’s response to a customer’s chat widget

Every message that the bot sends will be received by Omni-channel Engagement Hub services by default. To make the bot’s response visible on the customer’s chat widget, you must set `deliveryMode` to `bridged` on the reply activity’s `ChannelData`. The following sample code illustrates how the reply sent by the bot can be shown on the customer's chat widget.
...
<!--from editor: Below, should there be a closing quotation mark after "BotHandoffTopic" where it says “BotHandoffTopic equals to “CreditCard” ?-->

 
The dictionary `contextVarPairs` contains all the Omni-channel context variable name value pairs that you want to update as part of the escalation request. The bot can also send an escalation summary that will be visible to the agent once they accept the escalated chat request. Here, “BotHandoffTopic” is the context variable name and “CreditCard” is the context variable value. If there is an agent queue with the rule “BotHandoffTopic equals to “CreditCard”, then this escalated chat will be routed to that queue. Here is sample code to call the escalate method.
