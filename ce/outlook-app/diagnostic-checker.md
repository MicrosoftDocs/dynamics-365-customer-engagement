...
Use the Outlook Checker to run a diagnostic check and scan for potential issues for Dynamics 365 App for Outlook. If you run into an issue, you can send the log files to your administrator to help troubleshoot the problem.
 ...
 1. **Export logs**: After you run the checker, export the error logs and send the log files to your support team or admin for futher troubleshooting.
 2. **Clear cache**: This will clear any errors in the App for Outlook cache. Cache will rebuilt the next time you login. 
 3. **Hide or Show**: Move the toggle to **Show** to add the checker icon appears on the command bar. If the toggle is set to **Hide** press **ALT** + **CTRL** + **Y** on your keyboard to bring up the **Outlook Checker**
