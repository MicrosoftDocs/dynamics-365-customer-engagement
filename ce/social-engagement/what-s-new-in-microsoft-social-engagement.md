...
- You can choose to render the charts with a filling patterns or solid colors. When selecting the filling patterns, data on maps will be represented in different shapes and not rely on color exclusively.
- You can navigate the maps in [!INCLUDE[pn_netbreeze_short](../includes/pn-social-engagement-short.md)] with a keyboard by selecting tab to reach the first data point on map.
- In activity maps, you can choose between visual map and data table. The map allows navigation by keyboard in cardinal directions.
- Improved navigation for post list and post details

To learn more, see our dedicated [documentation on accessibility for Microsoft Social Engagement](accessibility.md).

### Administrators now enabled to remove recipients from alerts

With an administrator configuration role in [!INCLUDE[pn_netbreeze_short](../includes/pn-social-engagement-short.md)], you can now remove recipients from alerts that were configured by other users. You can search for a specific email address and then [remove the recipient from all alerts](email-alerts.md) which send email to that address. Additionally, you can export a list of alerts this recipient is added to.
