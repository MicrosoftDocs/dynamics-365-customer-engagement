...
> [!NOTE]
>  The concept and operation of tenants and instances, though similar, differs between online and on-premises deployments of [!INCLUDE[pn_microsoftcrm](../includes/pn-dynamics-crm.md)] apps. This topic is for those administering [!INCLUDE[pn_CRM_Online](../includes/pn-crm-online.md)] apps deployments.  
...
## Uses for multiple instances  
 ...
 Multiple instances are needed when segregation is required of plugins, workflows, or admin resources that cannot be easily isolated by using business units in [!INCLUDE [pn-crm-shortest](../includes/pn-crm-shortest.md)] apps.  
  
<a name="BKMK_Single"></a>   
## A multi-instance deployment  
 A typical Customer Engagement apps deployment includes one tenant only. A tenant can include one or more Customer Engagement apps instances; however, a Customer Engagement apps instance is always associated with a single tenant.  
...
- A licensed Customer Engagement apps user can potentially access all the Customer Engagement apps instances associated with the tenant. Access is controlled by instance security group membership.  
  
- You can purchase additional instances through the Additional Instance Add-On. Additional instances can be added only to "paid" subscriptions - not trials or Internal Use Rights (IUR). If you purchased your Customer Engagement apps subscription through Volume Licensing, you must go through your Large Account Reseller (LAR) to purchase the additional instance. [!INCLUDE[proc_more_information](../includes/proc-more-information.md)] [Billing and subscription support](billing-support.md)  
...
## Add an instance to your subscription  
 For information about how to add an instance to your Customer Engagement apps subscription, see [Add an instance to your subscription](../admin/add-instance-subscription.md).  
...
- In a multi-tenant scenario, a licensed Customer Engagement apps user associated with a tenant can only access one or more Customer Engagement apps instances mapped to the same tenant. To access another tenant a user would need a separate license and a unique set of sign-in credentials for that tenant.  
  
   For example, if User A has an account to access Tenant A their license allows them to access any and all instances created within Tenant A - if they are allowed by their administrator.  If User A needs to access instances within Tenant B, they will need an additional Customer Engagement apps license.  
...
- Multiple instances within a tenant are visible from the Customer Engagement apps interface if the administrator has access.  
