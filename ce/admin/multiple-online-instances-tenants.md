...
## A multi-instance deployment
 A typical Customer Engagement apps deployment includes one tenant only. A tenant can include one or more Customer Engagement instances; however, a Customer Engagement instance is always associated with a single tenant.
...
- A licensed Customer Engagement apps user can potentially access all the Customer Engagement instances associated with the tenant. Access is controlled by instance security group membership.
...
- In a multi-tenant scenario, a licensed Customer Engagement apps user associated with a tenant can only access one or more Customer Engagement instances mapped to the same tenant. To access another tenant a user would need a separate license and a unique set of sign-in credentials for that tenant.
...
### See also
 ...
 [Using multi-tenancy in Microsoft Dynamics CRM 2013 to address challenges in enterprise business environments (primarily on-premises)](https://www.microsoft.com/download/details.aspx?id=36056)
