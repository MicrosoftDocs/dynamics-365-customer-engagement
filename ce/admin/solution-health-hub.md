...
validate the configuration of the environment which may change over time through
...
potentially problematic configuration from which an app maker may want
to protect users.
...
Within the analysis job record that is created, a **Status** will indicate whether
or not the job has completed.

Once complete, the rules within the set will show in the **Analysis Results** grid.

 > [!div class="mx-imgBorder"] 
 > ![](media/solution-health-hub-3.png  "Analysis results")

In each analysis result is the following information:
...
In the case where the **Return Status** indicates **Fail**, the analysis results may
...
issues, there is the possibility to resolve each failure. To do this, select the
analysis results detail and then select **Resolve**.

 > [!div class="mx-imgBorder"] 
 > ![](media/solution-health-hub-4.png  "Analysis results resolve")

Analysis results details have specific information that helps highlight the details of the failure. In most cases the failure occurs because the records are configured incorrectly.
